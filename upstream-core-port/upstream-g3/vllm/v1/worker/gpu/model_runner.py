...
def _diff_sync_ckpt(label: str) -> None:
    """[DIFF-DUMP] SAMPLER-CKPT: marker-gated device sync checkpoint so a
    device-side assert from an async/unchecked launch surfaces in the exact
    phase that produced it (the IndexKernel.cu:111 assert of the Xid-31/43
    hunt). Zero-cost when the marker is absent: the marker existence check
    is cached per process (the probe used to stat /cache/.diff-dump on every
    call -- 8+ per-step call sites -- which was a per-step syscall in
    production)."""
    global _DIFF_SYNC_CKPT_ENABLED
    if _DIFF_SYNC_CKPT_ENABLED is None:
        try:
            import os as _os

            _DIFF_SYNC_CKPT_ENABLED = _os.path.exists("/cache/.diff-dump")
        except Exception:
            _DIFF_SYNC_CKPT_ENABLED = False
    if not _DIFF_SYNC_CKPT_ENABLED:
        return
    try:
        import torch as _torch

        _torch.cuda.synchronize()
        global _DIFF_SYNC_CKPT_N
        _DIFF_SYNC_CKPT_N = globals().get("_DIFF_SYNC_CKPT_N", 0) + 1
        if _DIFF_SYNC_CKPT_N % 800 == 0:
            print(
                f"[DIFF-DUMP] SAMPLER-CKPT n={_DIFF_SYNC_CKPT_N} {label} ok",
                flush=True,
            )
    except Exception as _e:
        print(f"[DIFF-DUMP] SAMPLER-CKPT-FAULT {label}: {_e}", flush=True)
        raise

_DIFF_SYNC_CKPT_ENABLED: bool | None = None



# [DIFF-DUMP] differential-harness instrumentation for the upstream-core port
# bisect: eager per-step checksum of the post-forward hidden states. Gated by
# a marker file on the host-writable /cache volume; zero-cost when absent.
# Mirrors the identical block in the r7/r7-legacy runners and the indexer
# builders so both stacks emit the same tags.
...
def _diff_dump_enabled() -> bool:
    ...
    if _DIFF_DUMP_ENABLED is None:
        try:
            ...
            _DIFF_DUMP_ENABLED = _os.path.exists("/cache/.diff-dump")
        except Exception:
...
def _kda_inputs_debug() -> bool:
    """VLLM_DEBUG_KDA_INPUTS=1 gate, cached per process (the GATHER AUDIT
    used to import os + read the env on every prepare_attn call)."""
    ...
    if _KDA_INPUTS_DEBUG_ENABLED is None:
        try:
            ...
            _KDA_INPUTS_DEBUG_ENABLED = bool(
                int(_os.environ.get("VLLM_DEBUG_KDA_INPUTS", "0"))
            )
        except Exception:
            _KDA_INPUTS_DEBUG_ENABLED = False
    return _KDA_INPUTS_DEBUG_ENABLED
...
def _diff_dump_hs(tag: str, tensor: torch.Tensor) -> None:
    with torch.no_grad():
        tf = tensor.detach().float()
        print(
            f"[DIFF-DUMP] {tag} sum={tf.sum().item():.4f} "
            f"absmean={tf.abs().mean().item():.6f} shape={tuple(tensor.shape)}",
            flush=True,
        )
...
class GPUModelRunner(LoRAModelRunnerMixin):
    def __init__(self, vllm_config: VllmConfig, device: torch.device):
        ...
        # [FORK-COMPAT] in-image v1/worker/gpu/warmup.py::warmup_kernels()
        # reads is_encoder_only unguarded at boot (v84 stock V2 runner sets it
        # in __init__ from VllmConfig.is_encoder_only).
        ...
        # [FORK-COMPAT] in-image v1/worker/gpu/warmup.py::warmup_kernels()
        # reads verification_capacity_manager unguarded at boot. v84 stock
        # default is None; only DSpark's capacity-based verification
        # constructs a manager, which this runner does not build.
    ...
    def load_model(self, load_dummy_weights: bool = False, *args, **kwargs) -> None:
        ...
        # [FORK-COMPAT] v84 ParallelConfig lacks enable_batch_sharded_sampling
        # (upstream-only field); default False = stock single-shard sampling.
        if getattr(self.parallel_config, "enable_batch_sharded_sampling", False):
        ...
        if self.is_last_pp_rank and not self.is_pooling_model:
            # [FORK-COMPAT] the in-image fork Sampler accepts a subset of
            # upstream's kwargs; pass only what the installed signature takes.
            _sampler_kwargs = {
                "max_num_reqs": self.max_num_reqs,
                "vocab_size": self.vocab_size,
                "device": self.device,
                ...
                "enable_trace_replay": getattr(
                    self.model_config, "enable_trace_replay", False
                ),
                "reasoning_config": self.vllm_config.reasoning_config,
                "return_sampling_mask": getattr(
                    self.model_config, "return_sampling_mask", False
                ),
            }
            import inspect as _inspect

            _accepted = set(
                _inspect.signature(Sampler.__init__).parameters.keys()
            )
            _accepted.discard("self")
            self.sampler = Sampler(
                **{k: v for k, v in _sampler_kwargs.items() if k in _accepted}
            )
            ...
            # [FORK-COMPAT] in-image StructuredOutputsWorker may not accept
            # upstream kwargs; pass only what the installed signature takes.
            _so_kwargs = {
            ...
            _accepted = set(
                _inspect.signature(
                    StructuredOutputsWorker.__init__
                ).parameters.keys()
            )
            _accepted.discard("self")
            self.structured_outputs_worker = StructuredOutputsWorker(
                **{k: v for k, v in _so_kwargs.items() if k in _accepted}
            )
    ...
    def initialize_kv_cache(
        ...
        # [ALIGN-RESYNC] Per-group mask of mamba "align" groups: the manager
        # nulls/reorders align-state blocks mid-request, so these block-table
        # rows are rewritten from the scheduler's full manager truth instead
        # of being append-only delta logs.
        def _group_is_mamba_align(group) -> bool:
            ...
            return any(
                isinstance(sp, MambaSpec) and sp.mamba_cache_mode == "align"
                for sp in specs
            )

        self.align_resync_groups = tuple(
            _group_is_mamba_align(group)
        ...
        # [CROSS-GROUP-LEAK hardening] Per-group pool block counts, derived
        # from the ACTUAL BOUND CACHE TENSORS - the exact tensors the
        # kernels index into (zero size arithmetic, zero matching
        # heuristics): after bind, each group's layer in the forward
        # context exposes its kv_cache; pool = that tensor's shape[0]
        # (the bound-tensor rows: the mamba align group's 198, the
        # indexer's real pool, the tail's own). The group's pool is the
        # MINIMUM over its bound layers (all layers of a group share one
        # tensor, so any layer is ground truth; the min keeps the bound
        # conservative if a wrapper diverges). Groups with no bound tensor
        # stay unbounded (the hardening is inert for them).
        try:
            forward_ctx = self.compilation_config.static_forward_context
            layer_pools: dict[int, int] = {}
            for group_idx, group in enumerate(kv_cache_config.kv_cache_groups):
                pools: list[int] = []
                for name in group.layer_names:
                    layer = forward_ctx.get(name)
                    kv = getattr(layer, 'kv_cache', None)
                    if kv is None:
                        continue
                    # Mamba hybrid layers bind a LIST of state tensors
                    # [num_blocks, ...] (conv + temporal); attention/indexer
                    # layers bind a single cache tensor. Handle both.
                    if isinstance(kv, (list, tuple)):
                        for st in kv:
                            if st is not None and st.ndim >= 1:
                                pools.append(int(st.shape[0]))
                    elif hasattr(kv, 'shape') and kv.ndim >= 1:
                        pools.append(int(kv.shape[0]))
                if pools:
                    layer_pools[group_idx] = min(pools)
            pool_num_blocks: list[int | None] = [
                layer_pools.get(gi)
                for gi in range(len(kv_cache_config.kv_cache_groups))
            ]
            self._pending_pool_num_blocks = pool_num_blocks
        except Exception:
            # Sizing is best-effort hardening; never block boot on it.
            pass
        if os.environ.get("VLLM_DEBUG_KDA_INPUTS", "0") == "1":
            import logging

            logging.getLogger(__name__).warning(
                "[GROUP-ORDER dump] kv_cache_groups: %s ; align_resync_groups:"
                " %s ; pool_num_blocks: %s",
                [
                    (
                        i,
                ...
                self.align_resync_groups,
                getattr(self, "_pending_pool_num_blocks", None),
            )
        # [CROSS-GROUP-LEAK hardening] Attach (post-bind, sanity-guarded):
        # every derived pool must cover its group's own per-request row
        # width, else the attach is abandoned entirely (fail-safe,
        # byte-identical behavior) rather than risk skipping legitimate
        # writes.
        try:
            pending = self._pending_pool_num_blocks
            if pending is not None:
                for gi, pool in enumerate(pending):
                    if pool is not None:
                        width = self.block_tables.block_tables[gi].gpu.shape[-1]
                        if pool < width:
                            pending = None
                            break
            if pending is not None:
                self.block_tables.pool_num_blocks = pending
        except Exception:
            pass
    ...
    def profile_single_request_prefill(self) -> None:
        """[FORK-COMPAT] v84 in-image gpu_worker calls this fork-only hook.

        Profiles the max single-request prefill shape before KV sizing (the
        sparse-attention workspace peak for one request holding the entire
        token budget). Delegates to upstream's _dummy_run machinery with the
        same flow as the v84 fork method it replaces.
        """
        try:
            from vllm.compilation.interface import set_current_vllm_config
        except ImportError:
        ...
        try:
            ...
            if getattr(self, "is_pooling_model", False):
                self._dummy_pooler_run(model_output[0])
        finally:
            try:
    ...
    def _sync_device(self) -> None:
        """[FORK-COMPAT] v84 profiling-flow device sync hook."""
        torch.accelerator.synchronize()

    def _init_minimal_kv_cache_for_profiling(self) -> None:
        """[FORK-COMPAT] the v84 gpu_worker boot path
        (_profile_model_with_kernel_warmup -> profile_single_request_prefill)
        needs a minimal KV cache so the attention-inclusive profile run
        exercises real block tables / slot mappings before the production KV
        cache is sized. Delegates to the staged cudagraph_utils minimal-config
        machinery (the same construction profile_cudagraph_memory uses)."""
        from vllm.v1.worker.gpu.cudagraph_utils import (
            _init_minimal_kv_cache_for_profiling as _do_init,
        )

        _do_init(self)

    def _cleanup_profiling_kv_cache(self) -> None:
        """[FORK-COMPAT] release the profiling KV cache / graphs captured
        for profile_single_request_prefill so the production
        initialize_kv_cache starts from a clean slate."""
    ...
    def _dummy_run(
        ...
        **kwargs,
        ...
        # [FORK-COMPAT] v84 _dummy_run single_request_prefill handling: one
        # request holds the whole token budget so the profile covers the
        # sparse-attention single-request workspace peak.
    ...
    def update_pp_decode_requests(self):
        ...
        _diff_sync_ckpt("pre-postprocess")
    ...
    def update_requests(self, scheduler_output: SchedulerOutput) -> None:
        ...
        align_full_ids = getattr(reqs, "align_full_block_ids", None)
        for i, (req_id, num_computed_tokens, req_new_block_ids) in enumerate(
            zip(reqs.req_ids, reqs.num_computed_tokens, reqs.new_block_ids)
        ):
            req_index = self.req_states.req_id_to_index[req_id]
            num_computed_tokens_np[req_index] = num_computed_tokens
            align_full = (
                align_full_ids[i]
                if align_full_ids is not None
                else None
            )
            if align_full is not None:
                # [ALIGN-RESYNC] Rewrite the align group rows from the
                # scheduler's full manager truth; other groups keep the
                # append contract, byte-identical to the non-resync path.
                self.block_tables.append_block_ids(
                    req_index,
                    req_new_block_ids,
                    overwrite=False,
                    rewrite_ids=align_full,
                )
    ...
    def prepare_inputs(
        ...
        input_batch = InputBatch(
            ...
            # [FORK-COMPAT] v84 in-image model states (v1/worker/gpu/
            # model_states/{default,encoder_decoder}.py) read these fields;
            # v84 model_runner computed them exactly this way.
    ...
    def prepare_attn(
        ...
        # [GATHER AUDIT] the persistent tables are validated at write time
        # (every step) and stay clean, yet the builder has read MLA-scale ids
        # from its gathered slice — validate the GATHERED tables (what the
        # builders actually consume) against each group's pool bound.
        # [GATHER AUDIT] env gate cached per process (was an import +
        # environ.get on every prepare_attn call -- once per step).
        if (
            _kda_inputs_debug()
            and getattr(self.block_tables, "pool_num_blocks", None)
        ):
            try:
                import torch as _torch_ga

                if _torch_ga.cuda.is_current_stream_capturing():
                    pass
                else:
                    _pools = self.block_tables.pool_num_blocks
                    for _gi in range(len(block_tables)):
                        _pool = _pools[_gi] if _gi < len(_pools) else None
                        if _pool is None:
                            continue
                        _t = block_tables[_gi]
                        if _t.ndim != 2 or _t.shape[0] == 0:
                            continue
                        _max = int(_t.abs().max().item())
                        if _max >= int(_pool):
                            _row = (_t.abs() >= int(_pool)).any(dim=1)
                            _bad = int(_row.nonzero()[0].item())
                            raise RuntimeError(
                                "[GATHER-AUDIT] group "
                                f"{_gi} gathered table row {_bad} max={_max} "
                                f">= pool={_pool} — the GATHER (idx_mapping="
                                f"{input_batch.idx_mapping.tolist()}) or its "
                                "source row is corrupt; the persistent tables "
                                "were clean at write time."
                            )
            except RuntimeError:
                raise
            except Exception:
    ...
    def sample(
        ...
        if grammar_output is not None:
            ...
            _so_params = set(
                _inspect_so.signature(
                    self.structured_outputs_worker.apply_grammar_bitmask
                ).parameters.keys()
            )
            _so_extra = {}
            if "grammar_num_spec_tokens" in _so_params:
                _so_extra["grammar_num_spec_tokens"] = getattr(
    ...
    def _merge_ec_connector_no_forward(
        ...
        # [FORK-COMPAT] v84 in-image ECConnector (ec_connector.py) has no
        # no_forward() — only maybe_get_output(); stock v84 idle steps
        # exchange nothing, so fall through with no EC payload.
        _no_forward = getattr(self.ec_connector, "no_forward", None)
        if _no_forward is not None:
            ec_out = _no_forward(scheduler_output).ec_connector_output
        else:
            ec_out = None
        return ModelRunnerOutput.with_ec_conn_output(output, ec_out)
    ...
    def execute_model(
        ...
        if self.uses_inputs_embeds and self.is_first_pp_rank:
            ...
            if dummy_run:
                ...
                with self.ec_connector.maybe_get_output(
                    ...
                    # [FORK-COMPAT] v84 in-image model states expose
                    # get_mm_embeddings instead of upstream's
                    # prepare_inputs_embeds; v84 stock path verbatim.
                    _prepare_inputs_embeds = getattr(
                        self.model_state, "prepare_inputs_embeds", None
                    )
                    if _prepare_inputs_embeds is not None:
                        inputs_embeds = _prepare_inputs_embeds(
        ...
        if batch_desc.cg_mode == CUDAGraphMode.FULL:
            ...
            with set_forward_context(
                ...
                # [DIFF-DUMP] eager post-forward checksum (see helpers at top).
                if _diff_dump_enabled():
                    _diff_step = _diff_step_counter("run-fwd")
                    if _diff_step >= 0:
                        if isinstance(model_output, tuple):
                            _hs = model_output[0]
                        else:
                            _hs = model_output
                        _diff_dump_hs(f"RUN-FWD step={_diff_step}", _hs)
    ...
    def sample_tokens(
        ...
        _diff_sync_ckpt("after-async-output")
        ...
        _diff_sync_ckpt("after-mm-gather")
        ...
        if self.speculator is not None and num_spec_tokens_to_schedule > 0:
            ...
            with use_workspace_lane(self._draft_workspace_lane):
                ...
                _diff_sync_ckpt("post-propose-limit")
            num_draft_tokens = draft_tokens.shape[1]
            if num_draft_tokens > 0:
                # [DIFF-DUMP] DRAFT-IDX: the faulting tiny gather window (the
                # post-draft-put SAMPLER-CKPT-FAULT of the Xid-31/43 hunt):
                # print the EXACT idx_mapping values feeding the gather plus
                # the target geometry, marker-gated via the cached per-process
                # gate (the raw exists() stat ran once per draft step; also
                # capture/warmup-skipped now, which the tolist sync requires).
                try:
                    if _diff_dump_enabled():
                        print(
                            f"[DIFF-DUMP] DRAFT-IDX "
                            f"ptr={input_batch.idx_mapping.data_ptr():#x} "
                            f"idx_mapping={input_batch.idx_mapping.tolist()}"
                            f" draft={tuple(draft_tokens.shape)} "
                            f"target={tuple(self.req_states.draft_tokens.shape)}",
                            flush=True,
                        )
                except Exception:
                    pass
                ...
                _diff_sync_ckpt("post-draft-put")
            ...
            if self.adaptive_verification is not None:
                ...
                _diff_sync_ckpt("post-record-conf")
        ...
        if self.num_speculative_steps > 0:
            ...
            _diff_sync_ckpt("post-set-draft")
