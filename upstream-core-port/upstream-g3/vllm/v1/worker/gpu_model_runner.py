...
# Mirrors the identical block in the r7 gpu_model_runner.py and the indexer
...
class GPUModelRunner(
    ...
    def _init_model_kwargs(self):
        ...
        # [H2D LIFETIME FIX - Xid-31/43 torn-host-source class] The pinned
        # staging temp dies at return; a non_blocking H2D can still be in
        # flight and the recycled pinned block feeds the GPU torn bytes.
        # This tensor is at most max_num_batched_tokens int32s (~KB), so
        # copy synchronously and close the window deterministically.
    ...
    def _update_states(self, scheduler_output: "SchedulerOutput") -> Callable | None:
        ...
        for i, req_id in enumerate(req_data.req_ids):
            ...
            align_full = (
                req_data.align_full_block_ids[i]
                if getattr(req_data, "align_full_block_ids", None) is not None
                else None
            )
            if not resumed_from_preemption:
                ...
                if align_full is not None:
                    # [ALIGN-RESYNC] The align manager has nulled/reordered
                    # state blocks mid-request; replace the stale worker-side
                    # rows with the scheduler's full manager truth.
                    for g, is_align in enumerate(self.align_resync_groups):
                        if is_align:
                            req_state.block_ids[g][:] = align_full[g]
            else:
            ...
            if align_full is not None:
                # [ALIGN-RESYNC] Rewrite the align group rows from manager
                # truth (full-row write); all other groups keep the append
                # contract, byte-identical to the non-resync path.
                single_tables = self.input_batch.block_table.block_tables
                for g, (single_table, is_align) in enumerate(
                    zip(single_tables, self.align_resync_groups)
                ):
                    if is_align:
                        single_table.add_row(align_full[g], req_index)
                    elif new_block_ids is not None:
                        single_table.append_row(new_block_ids[g], req_index)
            elif new_block_ids is not None:
    ...
    def _prepare_input_ids(
        ...
        # [H2D LIFETIME FIX - Xid-31/43 torn-host-source class] The pinned
        # staging temp below dies at scope exit; a non_blocking H2D could
        # still be in flight and the recycled pinned block feeds the GPU
        # torn index bytes -> OOB scatter_ writes (Xid 31/43 assert
        # family). These index tensors are at most a few dozen int64s
        # (<1KB), so stage them SYNCHRONOUSLY (~10us each): closes the
        # tear window deterministically (same pattern as
        # buffer_utils.async_copy_to_gpu small-metadata arm).
        sampled_tokens_index_tensor = torch.tensor(
            sample_flattened_indices, dtype=torch.int64, pin_memory=PIN_MEMORY
        ).to(self.device)
        # [H2D LIFETIME FIX] sync staging, see note above.
    ...
    def _build_attention_metadata(
        ...
        cached_attn_metadata: dict[tuple[Any, ...], AttentionMetadata] = {}
        ...
        def _build_attn_group_metadata(
            ...
            # [STALE-GRID fix] The update_block_table fast path refreshes only
            # block-id-derived fields; the spec/non-spec query grids and the
            # composition (prefill/decode/spec counts) are snapshots from the
            # cached build. Reusing a cached metadata across a composition
            # change therefore feeds a stale spec grid to the layer (e.g. a
            # global [2999, 3000] slice reaching a 2999-token-prefill +
            # 1-spec-token batch whose grid must be [0, 1]), which the KDA
            # kernels consume as token offsets -> wild VIRT_READ/WRITE
            # (Xid 31/43). Key the cache on the CURRENT batch's layout and
            # spec classification so a composition change falls through to a
            # full build(); the fast path stays for layout-identical replays.
            query_layout = tuple(
    ...
    def _gather_mm_embeddings(
        ...
        # [H2D LIFETIME FIX - Xid-31/43 torn-host-source class] is_mm_embed
        # used to be returned as a pinned CPU tensor; every consumer
        # (interfaces._embed_text_input_ids mask copy, _merge_multimodal_
        # embeddings boolean index, the MTP drafter embed path) then
        # re-copied it to the GPU with non_blocking H2D DMAs that could
        # still be in flight when the caller scope (_preprocess / drafter
        # propose) exited and the pinned block was recycled - the GPU
        # received a torn mask that unmasked OOV placeholder ids (OOB
        # embedding gather -> device assert) or scrambled the mm splice.
        # The mask is at most max_num_batched_tokens bools (a few KB), so
        # stage it to the GPU SYNCHRONOUSLY here: the copy completes before
        # return and no downstream H2D of this buffer can exist.
        return mm_embeds, is_mm_embed.to(self.device)
    ...
    def _model_forward(
        ...
        **model_kwargs: dict[str, Any],
        ...
        if _diff_dump_enabled():
            ...
            if _diff_step >= 0:
                if self.use_aux_hidden_state_outputs and isinstance(out, tuple):
                    _hs = out[0]
                else:
                    _hs = out
    ...
    def _is_uniform_decode(
        ...
        uniform_decode_query_len * num_reqs. [FORK-COMPAT v84] upstream
        dropped this guard; without it a mixed batch whose totals coincide
        (e.g. 7 decode reqs at 4 tokens + one 4-token prefill chunk) is
        dispatched onto the FULL decode cudagraph with prefill-shaped
        metadata.
        """
    ...
    def _dummy_run(
        ...
        if create_mixed_batch:
            ...
            # [FORK-COMPAT] v84 _dummy_run single_request_prefill branch
            # (gpu_model_runner.py:6120): one prefill request holding the
            # whole token budget, consumed by profile_single_request_prefill.
            assert not create_mixed_batch
            num_reqs = 1
    ...
    def profile_single_request_prefill(self) -> None:
        """[FORK-COMPAT] in-image gpu_worker._profile_model_with_kernel_warmup
        calls this hook on the runner unconditionally at boot; v84 stock V1
        defines it (gpu_model_runner.py:6719). Ported against the staged V1
        machinery (_init_minimal_kv_cache_for_profiling / _sync_device /
        _cleanup_profiling_kv_cache already present on this class).
        """
        ...
        for layer_name, attn_module in attn_layers.items():
            ...
            if spec := attn_module.get_kv_cache_spec(self.vllm_config):
                if isinstance(spec, AttentionSpec):
                    _backend_cls = attn_module.get_attn_backend()
                    if hasattr(_backend_cls, "customize_spec"):
                        spec = _backend_cls.customize_spec(spec)
