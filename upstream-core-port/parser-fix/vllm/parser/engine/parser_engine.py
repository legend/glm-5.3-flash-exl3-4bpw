        ...
        if not slot.name_sent:
            ...
            if name:
                if not self._accept_tool_name(name):
                    # A completed call to an undeclared tool must still
                    # reach the client: the client decides validity and can
                    # reply "no such tool", keeping agentic loops recoverable
                    # instead of silently swallowing the call. Mid-stream
                    # gating (truncated prefixes, #39757) is unaffected.
                    logger.warning_once(
                        "Tool call %r is not declared in the request tools; "
                        "emitting the completed call anyway.", name)
                slot.name = name
                slot.name_sent = True
                slot.string_keys = self._streamable_string_keys(
                    find_tool_properties(self._tools, name)
