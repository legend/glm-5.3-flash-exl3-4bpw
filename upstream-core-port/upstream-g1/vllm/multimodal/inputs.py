...
# [H2D LIFETIME FIX - Xid-31/43 torn-host-source class] Host leaves staged
# by reduce_data are freshly pinned TEMPORARIES: the non_blocking H2D below
# used to be the last reference, so the pinned block died at return while
# the DMA was still in flight, the host allocator recycled it, and the mm
# encoder received torn pixels/indices. Small (and unpinned) leaves now
# stage SYNCHRONOUSLY; large pinned leaves keep the async arm with an
# event-gated hold (same pattern as utils.torch_utils.async_tensor_h2d /
# v1.worker.gpu.buffer_utils.async_copy_to_gpu) so the block is never
# recycled before its DMA completes.
_MM_ASYNC_H2D_INFLIGHT: list[tuple[torch.Tensor, torch.cuda.Event]] = []
...
def _nested_tensors_h2d(
    ...
    def _h2d_leaf(x):
        if not isinstance(x, torch.Tensor):
            return x
        if not x.is_cpu or torch.device(device).type != "cuda":
            # Already on an accelerator (or a CPU target): no host DMA
            # window exists.
            return x.to(device=device)
        nbytes = x.numel() * x.element_size()
        if nbytes <= 4096 or not x.is_pinned():
            # Unpinned pageable sources cannot re-enter the pinned pool, and
            # small leaves are KB-scale; copy synchronously and close the
            # window deterministically.
            return x.to(device=device)
        result = x.to(device=device, non_blocking=True)
        ev = torch.cuda.Event()
        ev.record()
        _MM_ASYNC_H2D_INFLIGHT.append((x, ev))
        if len(_MM_ASYNC_H2D_INFLIGHT) > 64:
            _MM_ASYNC_H2D_INFLIGHT[:] = [
                (t, e) for (t, e) in _MM_ASYNC_H2D_INFLIGHT if not e.query()
            ]
        return result

    return json_map_leaves(_h2d_leaf, tensors)
