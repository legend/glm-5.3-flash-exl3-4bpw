...
def create_kv_cache_views(
    ...
    # [FORK-COMPAT] Compressed specs (tokens_per_state > 1, e.g. the GLM
    # kpool indexer at 4) must be split at the COMPRESSED kernel block:
    # the storage rows per view block must be in {32, 64} (the b12x
    # paged-MQA C++ contract, attention.hpp:247/344). The plain 64-token
    # kernel block yields 64//4 = 16 rows here -> block_kv=16 -> assert.
    # Mirrors the v84 compressed_kernel_block_size rescale at the builder
    # layer (worker/utils.py) for the view layer.
    if (
        kernel_block_size is not None
        and isinstance(spec.tokens_per_state, int)
        and spec.tokens_per_state > 1
    ):
        _storage = spec.block_size // spec.tokens_per_state
        for _cand in (64, 32):
            if _storage % _cand == 0:
                kernel_block_size = _cand * spec.tokens_per_state
                break
    ...
    if ratio > 1:
        ...
        # so they sit a constant stride apart only if blocks are stored as
        # dense, unpadded pages: the tensor's block stride must equal ONE
        # layer's page. The per-layer view then claims exactly num_blocks
        # pages starting at ``offset + l * layer_stride``, which is sound
        # only when the layer stride carries a whole per-layer region:
        # ``layer_stride >= num_blocks * dense_page_size`` — a layer-
        # contiguous per-bucket region tensor (block_stride = page,
        # layer_stride = num_blocks * page) or a layer-compact tensor.
        #
        # A per-block interleaved bucket tensor (layer_stride = one page,
        # block_stride = every bucket layer's pages) cannot express the
        # split at all: its kernel blocks stride whole-bucket pages apart,
        # and collapsing the view's block stride to the per-layer kernel
        # block while keeping the one-page layer stride made every layer
        # claim num_blocks pages from a base one page along — the 12
        # sparse-MLA layer views aliased and corrupted every context past
        # the first blocks (bases 0x225000 apart, each view shaped
        # (num_blocks * ratio, ...) at the per-kernel-block stride).
        ...
        assert dense_page_size % ratio == 0, (
            f"Page {dense_page_size} must divide into {ratio} equal kernel "
            "blocks."
        )
        assert layer_stride >= num_blocks * dense_page_size, (
            "Kernel-block splitting requires a whole per-layer region per "
            f"layer: layer_stride {layer_stride} < num_blocks * page "
            f"({num_blocks} * {dense_page_size}); a per-block interleaved "
            "bucket tensor cannot express the split."
        )
        # One layer's kernel blocks are a contiguous run of `ratio` equal
        # pieces inside its dense page, so the view's block stride is the
        # per-layer kernel block stride.
        block_stride = dense_page_size // ratio
...
class AttentionSpec(KVCacheSpec):
    ...
    def state_content_size_bytes(self) -> int:
        ...
        # [FORK-COMPAT] v84 packed layout for nvfp4_ds_mla: fp4 data
        # (dim/2 bytes) + fp8 block scales (dim/16 bytes) per head —
        # 512-dim head => 288 bytes/token, which the b12x kernel asserts
        # (kv_cache.py:577: "must be (num_blocks, block_size, 288) uint8").
        if getattr(self.kv_quant_mode, "is_nvfp4", False):
            last_dim = nvfp4_kv_cache_full_dim(
                self.head_size
            ) + nvfp4_kv_cache_full_dim(self.head_size_v)
            return last_dim * get_dtype_size(self.dtype)
...
class MLAAttentionSpec(FullAttentionSpec):
    ...
    head_size_v: int = 0
    # [FORK-COMPAT] v84 name for ``tokens_per_state``. The fork
    # ``Glm5NextIndexerCache.get_kv_cache_spec`` builds its spec via
    # ``dataclasses.replace(spec, compress_ratio=index_kpool)``, so the field
    # must remain an init kwarg. Kept in sync with ``tokens_per_state`` (the
    # upstream source of truth) in ``__post_init__``.
    ...
    def __post_init__(self):
        ...
        if self.compress_ratio is None:
            ...
            if self.tokens_per_state == 1:
                # The upstream field was left at its default; the fork kwarg
                # is the source of truth.
                object.__setattr__(self, "tokens_per_state", self.compress_ratio)
            else:
                raise ValueError(
                    "compress_ratio and tokens_per_state disagree: "
                    f"{self.compress_ratio} vs {self.tokens_per_state}"
                )
        _apply_alignment_padding(self)

    @property
    def storage_block_size(self) -> int:
        # [FORK-COMPAT] v84 field: token slots per stored index row.
        if self.tokens_per_state > 0:
...
# ----------------------------------------------------------------------
# [FORK-COMPAT] v84-fork symbols retained because fork-side files stay in
# the composed mount set (composition A: upstream CORE, fork MODEL side):
# glm5next attention.py (KpoolTailSpec producer, compress_ratio consumer),
# mla_attention, the mooncake connector (lazy, unused in our config), and
# the TQ/offloading config paths. The upstream core never reads these;
# validated against UpstreamG4's simulated interface (13/13 import checks).
# ----------------------------------------------------------------------
...
class KpoolTailSpec(SlidingWindowSpec):
    ...
    def __post_init__(self):
        # Pack the constructor's head_size (== 2 * head_dim: raw K + gate
        # score) as TWO head slots of head_dim each -- the
        # [num_blocks, 2, kpool, head_dim] view contract the fork-side
        # KpoolTailBackend.get_kv_cache_shape publishes and the in-image
        # kpool_compress kernels assert (kpool_compress.py:852-856). The
        # generic [B, H, N, C] view builder (compute_layer_kv_cache_shape_bytes
        # + create_kv_cache_views) reads H = num_head_slots and C =
        # state_content_bytes; left at the AttentionSpec defaults they give
        # H = num_kv_heads = 1 and C = head_size + head_size_v = 2 * head_dim,
        # i.e. a [num_blocks, 1, kpool, 2*head_dim] view that dies at the
        # boot assert. The unpadded page is unchanged
        # (2 * kpool * head_dim * dtype_size), so page math, group padding and
        # tensor emission are byte-identical.
        assert self.head_size % 2 == 0 and self.head_size_v == 0, (
            "KpoolTailSpec expects head_size == 2 * head_dim with head_size_v=0"
        )
        object.__setattr__(self, "num_head_slots", 2)
        object.__setattr__(
            self,
            "state_content_bytes",
            self.head_size // 2 * get_dtype_size(self.dtype),
        )
        super().__post_init__()
    ...
    def max_num_blocks_per_req(self, vllm_config: VllmConfig, max_len: int) -> int:
        ...
        # per-request block-table buffer the runner sizes via this method.
        return 1
...
def get_mla_kv_model_version(
    kv_cache_dtype: str, hf_text_config: Any
) -> str | None:
    """Return the cache-record ABI selected by semantic MLA dimensions."""
    if (
