...
class CpuGpuBuffer:
    ...
    def copy_to_gpu(self, n: int | None = None) -> torch.Tensor:
        # [H2D LIFETIME FIX - the Xid-31/43 producer class] The persistent
        # pinned CPU view is rewritten by the caller's NEXT step staging
        # while a non-blocking DMA from THIS call can still be in flight
        # (the CPU runs ahead of the GPU under async scheduling), so the
        # GPU received torn/stale rows (measured: per-step metadata with
        # freed-pointer-scale garbage driving IndexKernel device asserts
        # and the Xid-31/43 wild-write family). These buffers are small
        # per-step metadata; stage SYNCHRONOUSLY (~10us) and close the
