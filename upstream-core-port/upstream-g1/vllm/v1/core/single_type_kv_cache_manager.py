...
# [FORK-COMPAT] Manager for the GLM-5.3-Flash kpool tail group (composition A:
# upstream CORE, fork MODEL side -- the fork glm5next attention.py stays in-image and
# produces KpoolTailSpec groups). Rebased from the v84 overlay onto the upstream
# FullAttentionManager signatures; all override signatures verified identical.
...
def register_all_kvcache_specs(vllm_config):
    ...
    # [FORK-COMPAT] GLM-5.3-Flash kpool tail: its own manager, never grouped
    # with SlidingWindowSpec (SlidingWindowManager's mid-pool eviction would
    # destroy the in-progress pool; see KpoolTailManager).
    KVCacheSpecRegistry.register(
        KpoolTailSpec, KpoolTailManager, uniform_type_base_spec=KpoolTailSpec
    )
