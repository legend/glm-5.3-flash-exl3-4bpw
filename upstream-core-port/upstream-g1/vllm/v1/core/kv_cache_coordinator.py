...
class HybridKVCacheCoordinator(KVCacheCoordinator):
    ...
    def __init__(
        ...
        # [FORK-COMPAT] v84 coordinator assert (verbatim): the scheduling
        # granularity (LCM of all group block sizes) must be a multiple of
        # the hash_block_size and of each group's block size. The upstream
        # per-manager % hash form breaks on non-participating scratch groups
        # (GLM kpool tail, block_size == kpool == 4, hashes at its own
        # granularity via participates_in_prefix_caching=False).
        assert scheduler_block_size % hash_block_size == 0 and all(
            scheduler_block_size % bs == 0 for bs in group_block_sizes
        ), (
            "The scheduling granularity must be a multiple of hash_block_size "
            f"and of each group's block size. block_sizes={group_block_sizes}, "
        ...
        if self.enable_partial_hash_hits:
            unsupported_partial_hit_managers = {
                ...
                # Scratch groups that opted out of prefix caching (e.g. the
                # GLM-5.3-Flash kpool tail) never serve cache hits, so they
                # cannot constrain fine-grained hash lookups. Without this
                # filter, KpoolTailManager vetoed partial hash hits for the
                # whole engine, collapsing prefix-reuse granularity to
                # scheduler_block_size (15616 tokens under DCP2) and putting
                # every sub-15616-token shared prefix out of reach.
    ...
    def verify_and_split_kv_cache_groups(self) -> None:
        ...
        for i, g in enumerate(self.kv_cache_config.kv_cache_groups):
            ...
            # [FORK-COMPAT] Groups that opt out of prefix caching (e.g.
            # GLM-5.3-Flash kpool tail: per-request scratch, never shareable)
            # must not participate in hit lookup -- their manager-level hooks
            # already no-op, but entering the fixed-point loop would cap the
            # reconciled hybrid hit at 0. Their slot in the per-group hit
            # tuple stays empty.
