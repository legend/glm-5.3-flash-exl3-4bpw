...
def resolve_kv_cache_block_sizes(
    ...
    # [FORK-COMPAT] Scratch groups that opt out of prefix caching (e.g.
    # GLM-5.3-Flash kpool tail, block_size == kpool) must not constrain hash
    ...
    for layer_name, layer_spec in kv_cache_spec.items():
        if layer_spec.page_size_bytes == max_page_size:
            ...
            if max_page_size % layer_page_size == 0:
                ...
                and getattr(layer_spec, "indexes_kv_by_block_stride", False)
            ):
                # [FORK-COMPAT] v84 pads the GLM indexer k_cache (an
                # MLAAttentionSpec subclass) via the block-stride padding
                # path — the indexer's pages are indexed by block stride,
                # so a padded page is safe (v84 kv_cache_utils verbatim).
                new_spec = replace(layer_spec, page_size_padded=max_page_size)
            else:
                raise NotImplementedError(
                    f"Layer {layer_name}: page size {layer_page_size} is not "
                    f"divisible by the maximum page size {max_page_size} "
                    "(all page sizes: "
                    f"{sorted(set(page_sizes))}) and cannot be padded. "
                    "Padding is only supported for non-MLA attention layers."
                )
            assert new_spec.page_size_bytes == max_page_size
            new_kv_cache_spec[layer_name] = new_spec
...
def get_kv_cache_config_from_groups(
    ...
    for group in kv_cache_groups:
        ...
        # [GLM5Next] A group whose buckets have unequal page sizes (e.g. the
        # GLM-5.3-Flash attention group: kpool indexer pages beside MLA
        # latent pages) cannot keep the group-interleaved fixed block stride
        # and still store blocks as dense pages. Interleaving puts every
        # bucket at a bucket offset inside each manager block, so no uniform
        # stride addresses one bucket's blocks and kernel-block splitting
        # fails its density check. Emit one LAYER-CONTIGUOUS region per
        # bucket instead (v84's per-layer allocations): bucket layer l's
        # blocks are one contiguous run of num_blocks dense pages at
        # ``offset + l * num_blocks * page`` (block_stride = page,
        # layer_stride = num_blocks * page). Uniform-page groups keep the
        # interleaved path unchanged.
        per_bucket_layout = layout.is_block_outermost and len(
            {spec.page_size_bytes for spec in layers_by_spec}
        ) > 1

        byte_offset = 0
        for spec, layer_names in layers_by_spec.items():
            # [GLM5Next] Slot-sharing parasitic groups (the GLM-5.3-Flash
            # mamba groups whose pages the grouping padded to the MLA page,
            # and the kpool tail group whose pages it padded to the indexer
            # page) parasitize a position-twin bucket of the attention group
            # — v84's co-owned per-layer regions: MLA layer i's region is
            # co-owned by mamba layer i of every mamba group, each indexer
            # layer's region by its sibling tail layer. A host bucket is a
            # layer-contiguous region tensor (block_stride = page,
            # layer_stride = num_blocks * page), so a parasitic tensor is
            # collision-free when it COPIES the host geometry (same offset,
            # block_stride, layer_stride) and its layer count fits the
            # host's: parasitic layer m, id b then occupies page-units
            # {m * num_blocks + b} of the shared region while host layer i,
            # id g occupies {i * num_blocks + g}. Two distinct ids never
            # share a page-unit: u_m(b) == u_i(g) implies
            # (m - i) * num_blocks == g - b, and |g - b| < num_blocks forces
            # m == i and b == g — and the pool hands one id to at most one
            # group at a time. The parasitic footprint stays inside the host
            # region because m < len(host.layers), and parasitic layer m
            # addresses the SAME bytes as host layer m for the same id —
            # byte-equal twin pairing, exactly v84's co-owned tensors.
            # Misfit raises instead of emitting rows that would straddle
            # live host pages (the pre-fix interleaved straddle read MLA
            # fp4 pages as bf16/fp32 state -> inf/nan from the first warmup
            # forward -> exponential residual blowup).
            host = next(
                (
                    t
                    for t in kv_cache_tensors
                    if t.block_stride == spec.page_size_bytes
                    and t.layer_stride == num_blocks * spec.page_size_bytes
                ),
                None,
            )
            if isinstance(spec, (MambaSpec, KpoolTailSpec)) and host is not None:
                if len(layer_names) > len(host.layers):
                    raise ValueError(
                        f"Parasitic group ({len(layer_names)} layers) does "
                        "not fit its host bucket "
                        f"({len(host.layers)} layers); cannot slot-share."
                    )
                layer_stride = host.layer_stride
                block_stride = host.block_stride
                offset = host.offset
            elif per_bucket_layout:
                # Layer-contiguous per-bucket region. Bucket-major offsets
                # are unchanged: every preceding bucket filled num_blocks
                # blocks of its own pages, so the offset still scales by
                # num_blocks, and bucket 0 still aliases the region from
                # byte 0. The per-bucket regions tile the group's share of
                # the allocation exactly: sum(len(layers) * page) *
                # num_blocks bytes per group.
                #
                # The previous per-block interleave (block_stride =
                # len(layers) * page, layer_stride = page) cannot express a
                # valid per-layer flat view once kernel-block splitting
                # divides the page (create_kv_cache_views): the split view
                # strides kernel blocks at page//ratio with bases one page
                # apart, so every layer claimed num_blocks pages starting
                # one page along — the 12 sparse-MLA layer views overlapped
                # and corrupted every context past the first blocks.
...
def _get_kv_cache_groups_glm5_next(
    ...
    [FORK-COMPAT] Ported verbatim from the v84 fork (kv_cache_utils
    `_get_kv_cache_groups_glm5_next`): without it the upstream generic path
    page-size-unifies the MLA page with the kpool indexer page
    (2293760 % 147840 != 0) and dies on the un-padable MLA-typed indexer
    spec. The fork keeps the indexer spec's ``block_size`` at the global
    cache block (``compress_ratio``/``tokens_per_state`` pools storage),
    merges MLA + indexer into one UniformType attention group, pads the
    kpool tail group to the indexer page, and pads mamba pages to the MLA
    page so mamba groups share the MLA layers' slot tensors.
    """
    ...
    replicated_draft_specs = {
        k: v
        for k, v in kv_cache_spec.items()
        if not isinstance(v, (MambaSpec, KpoolTailSpec))
        and getattr(v, "dcp_replicated", False)
    }
    attn_specs = {
        k: v
        for k, v in kv_cache_spec.items()
        if not isinstance(v, (MambaSpec, KpoolTailSpec))
        and k not in replicated_draft_specs
    }
    if not mamba_specs or not all(
        type(s) is MLAAttentionSpec for s in attn_specs.values()
    ):
        return None
    # The upstream-core MLAAttentionSpec keeps the fork's ``compress_ratio``
    # init kwarg (set by the fork Glm5NextIndexerCache.get_kv_cache_spec via
    # ``replace(spec, compress_ratio=index_kpool)``) synced with the upstream
    # ``tokens_per_state``, so the fork's compress_ratio==1/>1 split holds.
    mla_specs = attn_specs
    ...
    draft_group = None
    if replicated_draft_specs:
        draft_uniform = UniformTypeKVCacheSpecs.from_specs(replicated_draft_specs)
        if draft_uniform is None:
            return None
        draft_group = KVCacheGroupSpec(
            list(replicated_draft_specs),
            draft_uniform,
            is_eagle_group=True,
        )
    ...
    # [FORK-COMPAT] v84 reads ``MambaSpec.real_page_size_bytes``; the
    # upstream-core MambaSpec computes the same unpadded page inline, and its
    # ``state_content_size_bytes`` property is exactly that value.
    if any_mamba.state_content_size_bytes > mla_page:
        raise ValueError(
            f"the mamba state page ({any_mamba.state_content_size_bytes} bytes) "
...
def get_kv_cache_groups(
    ...
    if is_kv_cache_spec_uniform(kv_cache_spec):
        ...
        # Must run before the DCP-replicated-draft split: the kpool tail specs
        # are dcp-replicated and must be padded to the indexer page here, not
        # split off as an unpadded replicated group.
        return glm5_groups
