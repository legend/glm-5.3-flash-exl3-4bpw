...
class Scheduler(SchedulerInterface):
    def __init__(
        ...
        # [FORK-COMPAT] v84 ModelConfig lacks uses_xdrope (GLM has none)
        self.model_uses_xdrope = getattr(vllm_config.model_config, "uses_xdrope", False)
        self.kv_cache_config = kv_cache_config
        self.kv_events_config = vllm_config.kv_events_config
        self.parallel_config = vllm_config.parallel_config
        self.log_stats = log_stats
        # [MANAGER-LIST dump] gate + step counter, hoisted out of the
        # per-request loop in _make_cached_request_data: the inline
        # `import os` + environ.get + two try/except frames ran per
        # running request PER STEP in the scheduler hot loop (pure
        # diagnostic-leftover cost when VLLM_DEBUG_KDA_INPUTS is unset).
        self._kda_manager_debug = (
            os.environ.get("VLLM_DEBUG_KDA_INPUTS", "0") == "1"
        )
        self._kda_manager_dump_step = 0
        self.observability_config = vllm_config.observability_config
        self.spec_decode_metrics_level = (
            getattr(self.observability_config, "per_request_spec_decode_metrics", False)
        )
        ...
        # [FORK-COMPAT] v84 VllmConfig lacks is_mm_encoder_only (upstream-only
        # property for mm-encoder-only runs); v84 stock is never encoder-only.
        self.is_mm_encoder_only = getattr(vllm_config, "is_mm_encoder_only", False)
        ...
        # [FORK-COMPAT] the in-image EncoderCacheManager has a plain
        # __init__(cache_size) (no create_manager classmethod); probe and
        # fall back to direct construction (v84 semantics).
        if hasattr(manager_cls_obj, "create_manager"):
        ...
        if speculative_config is not None:
            ...
            if (
                ...
                self.acceptance_length_controller = BatchSizeAcceptanceLengthController(
                    ...
                    initial_num_spec_tokens=getattr(
                        speculative_config,
        ...
        # nulls/reorders align-state blocks mid-request while the worker's
        # block-table rows are append-only delta logs, so their positions
        # diverge; these rows are rewritten from manager truth every step.
        ...
        self.align_resync_groups = tuple(
            ...
            for group in kv_cache_config.kv_cache_groups
        ) if self.need_mamba_block_aligned_split else ()
        ...
        # [FORK-COMPAT] v84 ModelConfig lacks return_sampling_mask
        self.return_sampling_mask = getattr(vllm_config.model_config, "return_sampling_mask", False)
    ...
    def _make_cached_request_data(
        ...
        # [ALIGN-RESYNC] Full manager truth for the align groups of persistent
        # requests; the worker rewrites those block-table rows every step.
        align_full_block_ids: list[tuple[list[int], ...] | None] | None = (
            [] if self.align_resync_groups else None
        )
        for idx, req in enumerate(itertools.chain(running_reqs, resumed_reqs)):
            ...
            if align_full_block_ids is not None:
                if idx < num_running_reqs:
                    # Persistent request: ship manager truth for align groups.
                    full_ids = self.kv_cache_manager.get_blocks(
                        req_id
                    ).get_block_ids(allow_none=True)
                    # [MANAGER-LIST dump] Under VLLM_DEBUG_KDA_INPUTS, dump
                    # the manager's own per-request align lists at the step
                    # they ship: if the MANAGER holds non-align-scale ids
                    # (values beyond its own group's state pool), the leak
                    # is in the scheduler/coordinator allocation side; if
                    # the lists are clean, the corruption is downstream
                    # (worker staging or the metadata gather). Sampled at
                    # most every 16 steps to bound the log volume.
                    # Gate + counter are hoisted to __init__ (cached env
                    # read) so this hot per-request path carries no
                    # import/environ/try-except overhead when the marker
                    # is off.
                    if (
                        self._kda_manager_debug
                        and not self._kda_manager_dump_step % 16
                    ):
                        _align_lists = [
                            (gi, full_ids[gi])
                            for gi, is_align in enumerate(
                                self.align_resync_groups
                            )
                            if is_align and full_ids[gi] is not None
                        ]
                        logger.warning(
                            "[MANAGER-LIST dump] req=%s align groups=%s",
                            req_id,
                            _align_lists[:256],
                        )
                    self._kda_manager_dump_step += 1
                    align_full_block_ids.append(
                        tuple(
                            group_ids if is_align else None
                            for group_ids, is_align in zip(
                                full_ids, self.align_resync_groups
                            )
                        )
                    )
                else:
                    # Resumed requests already carry full truth through
                    # new_block_ids; no rewrite signal is needed.
                    align_full_block_ids.append(None)
        ...
        return CachedRequestData(
            ...
            align_full_block_ids=align_full_block_ids,
    ...
    def update_from_output(
        ...
        for req_id, num_tokens_scheduled in num_scheduled_tokens.items():
            ...
            if should_emit_output:
                ...
                # [FORK-COMPAT] the in-image EngineCoreOutput is a msgspec
                # Struct accepting a subset of upstream's fields (it lacks
                # new_sampling_mask / spec_decode_metrics); pass only what
                # the installed signature takes.
                _eco_kwargs = {
                    "request_id": req_id,
                    "new_token_ids": new_token_ids,
                ...
                _accepted = set(
                    _inspect.signature(EngineCoreOutput).parameters.keys()
                )
        ...
        if (
            stats := self.make_stats(
                ...
                cudagraph_stats,
                perf_stats,
                # [FORK-COMPAT] v84 semantics, verbatim: context tokens
                # scheduled this iteration (consumed by in-image metrics
                # loggers for prompt throughput).
    ...
    def make_stats(
        ...
        perf_stats: PerfStats | None = None,
        # [FORK-COMPAT] v84 in-image v1/metrics/loggers.py reads
        # `scheduler_stats.num_scheduled_prompt_tokens` (prompt throughput);
        # v84's scheduler populated it from compute_iteration_details.
        ...
        return SchedulerStats(
            ...
            # [FORK-COMPAT] consumed by in-image v1/metrics/loggers.py
