...
def async_tensor_h2d(
    ...
    assert t.is_cpu
    # [H2D LIFETIME FIX - the Xid-31/43 producer class] Previously `t` (the
    # pinned staging copy) died at return while the non-blocking H2D was
    # still in flight: the pinned block returned to the host allocator, the
    # next same-size pin reused it, and the GPU received torn/stale bytes -
    # measured live in the 2026-09-03 Xid-31/43 hunt ([DIFF-DUMP]
    # DRAFT-IDX idx_mapping=[9143985375312] / [3440268804896], freed
    # host-pointer fragments). Small per-step metadata (grids, counts,
    # checkpoint columns - a few dozen bytes) now stages SYNCHRONOUSLY
    # (~10us, closes the window deterministically); large inputs keep the
    # async arm with an event-gated hold so the block is never recycled
    # before the DMA completes.
    nbytes = t.numel() * t.element_size()
    if nbytes <= 4096:
        return t.to(device=device, dtype=dtype)
    result = t.to(device=device, dtype=dtype, non_blocking=True)
    _ev = torch.cuda.Event()
    _ev.record()
    _ASYNC_H2D_INFLIGHT.append((t, _ev))
    if len(_ASYNC_H2D_INFLIGHT) > 64:
        _ASYNC_H2D_INFLIGHT = [
            (_e0, _e1) for (_e0, _e1) in _ASYNC_H2D_INFLIGHT if not _e1.query()
        ]
    return result
