...
# Native FP8 KV on the NoPE model (head 512): the validated r7 528B record
# ABI (pooled_indexer.py:64 _MLA_RECORD_BYTES=528; REFERENCE-VALIDATION.md
# §1.1). Default OFF -- bit-identical refusal (plus a warning log at the
# decision site) so fp8+head512 keeps the current FlashInfer redirect.
_B12X_FP8_KV_ENV = "VLLM_B12X_FP8_KV"
_GLM_NOPE_FP8_KV_RECORD_BYTES = 528


def _b12x_fp8_kv_enabled() -> bool:
    return os.environ.get(_B12X_FP8_KV_ENV, "0") == "1"
...
class B12xMLASparseBackend(AttentionBackend):
    ...
    def supports_combination(
        ...
        if head_size == _NOPE_Q_HEAD_DIM:
            if kv_cache_dtype == "fp8_ds_mla":
                if not _b12x_fp8_kv_enabled():
                    # The request falls through to the FlashInfer path, which
                    # crashes Xid 31 under prefix caching (8/8 faults, v75
                    # chain). Make the redirect loud instead of silent.
                    logger.warning(
                        "B12X_MLA_SPARSE: fp8_ds_mla + head_size=512 refused "
                        "by default; the request falls through to the "
                        "FlashInfer path, which crashes Xid 31 under prefix "
                        "caching (v75 prior-art chain). Set %s=1 to enable "
                        "the validated native 528B fp8_ds_mla record on the "
                        "b12x path.",
                        _B12X_FP8_KV_ENV,
                    )
                    return (
                        "B12X_MLA_SPARSE supports head_size=512 "
                        "(qk_rope_head_dim=0) with fp8_ds_mla only when "
                        f"{_B12X_FP8_KV_ENV}=1; got {kv_cache_dtype!r} with "
                        "the gate off"
                    )
                # Validated NoPE-FP8 ABI (528B record): admitted.
            ...
            elif not _GLM_NOPE_NVFP4_OPT_IN:
    ...
    def get_kv_cache_shape(
        ...
        if cache_dtype_str == "fp8_ds_mla":
            if head_size == _NOPE_Q_HEAD_DIM and _b12x_fp8_kv_enabled():
                # Validated GLM_NOPE FP8 record (pooled_indexer.py:64
                # _MLA_RECORD_BYTES=528): 512B e4m3 latent + 16B inline fp32
                # scales, NO rope tail (qk_rope_head_dim == 0). Width
                # authority: opt-work/fp8-native/staged/fp8_kv_record_abi.py.
                return (num_blocks, block_size, _GLM_NOPE_FP8_KV_RECORD_BYTES)
            # Rope-bearing V32 fp8_ds_mla packed: 656 B/token (512 NoPE +
            # 16 inline FP32 scales + 128 BF16 RoPE). Mirrors the FlashMLA /
            # SPARSE_MLA_SM120 layout; b12x's GLM_NSA decode reads the same
            # record. With the FP8 gate off this shape is unreachable for
            # head 512 (supports_combination refuses it).
...
@dataclass
class _B12xDraftUpdateState:
    """[FUSED-DRAFT-DECODE] Build-time stash for update_draft_decode_metadata.

    Refreshed by every build(); consumed only by the fused draft-decode loop
    (or the Tier-A shadow verifier) right after the draft step-1 build. The
    global seq_lens reference advances in place between draft steps; DCP
    local lengths are recomputed from it (never incremented: under
    round-robin interleave a +1 advance only lands on the rank owning the
    new global token index).
    """
...
class B12xMLASparseMetadataBuilder(AttentionMetadataBuilder[B12xMLASparseMetadata]):
    ...
    # [FUSED-DRAFT-DECODE] The update hook reproduces the decode fast path
    # of build() for one advanced draft position (see the hook below).
    # DCP-conditional: under DCP2+ the cache fields are persistent-buffer
    # copies the hook advances address-stably; under DCP1 they alias the
    # per-build [DRAFT-INPUT RECOVERY] clamp copy of input_buffers.seq_lens
    # (a fresh tensor every build) -- advancing them under FULL capture
    # would bake a stale heap address ([REVERT-20260904] class) and leaving
    # them frozen would feed the B12X decode kernels a stale causal bound.
    # Either way the fused path is unsafe under DCP1 -> report unsupported
    # so the speculator keeps the per-step rebuild fallback.
    _draft_decode_needs_live_seq_lens = True
    ...
    def __init__(
        ...
        # [FUSED-DRAFT-DECODE] refresh by every build(), see
        # _B12xDraftUpdateState.
        self._draft_decode_update_state: _B12xDraftUpdateState | None = None
        self._draft_decode_live_seq_lens: torch.Tensor | None = None
    ...
    def build(
        ...
        # [FUSED-DRAFT-DECODE] stash for update_draft_decode_metadata; see
        # _B12xDraftUpdateState. Uniform single-token decode shape only.
        self._draft_decode_update_state = None
        # Live seq-len source when bound (fused/shadow paths); fall back to
        # the cm view for gate-off fallback builds, whose hook never runs.
        live = self._draft_decode_live_seq_lens
        if cm.max_query_len <= 1 and num_tokens == cm.num_reqs:
            self._draft_decode_update_state = _B12xDraftUpdateState(
                ...
                global_seq_lens=(
                    live[: cm.num_reqs]
                    if live is not None
                    else cm.seq_lens[: cm.num_reqs]
                ),
                generic_slot_ref=cm.slot_mapping,
            )
    ...
    def update_draft_decode_metadata(
        ...
        """[FUSED-DRAFT-DECODE] Advance the B12X metadata one draft step.

        In-place through the tensors the metadata holds, capture-safe, and
        rebuild-equivalent with build()'s decode fast path:

        - slot_mapping aliases the generic BlockTables view that the fused
          loop's compute_slot_mappings already advanced (identity no-op in
          fused; refresh only matters for the shadow sandbox).
        - DCP: cache_seq_lens_per_token/req are COPIES of the rank-local
          lengths -- recomputed from the advanced global seq_lens via
          get_dcp_local_seq_lens (mathematically identical to the
          prepare_dcp_local_seq_lens triton the rebuild runs; round-robin
          interleave means a naive "+1" would be WRONG). DCP1: the fields
          are direct views of the advancing seq_lens (AUTO) -- the copies
          below are identity no-ops.
        """
        state = self._draft_decode_update_state
        if state is None or state.global_seq_lens is None:
            raise RuntimeError(
                "B12xMLASparseMetadataBuilder.update_draft_decode_metadata "
                "called without a bound live seq-len source (configure "
                "must rebind; the gate-off fallback never invokes this "
                "hook)"
            )
        if metadata.slot_mapping is not state.generic_slot_ref:
            metadata.slot_mapping.copy_(state.generic_slot_ref)
        if state.is_dcp:
            # RAW live source: the rebuild's dcp_local_seq_lens is prepared
            # from input_buffers.seq_lens BEFORE the [DRAFT-INPUT RECOVERY]
            # clamp (pad rows stay 0), so no clamp parity here -- a clamp
            # would lift pad rows and diverge on rank-0 pad rows.
            local = get_dcp_local_seq_lens(
                state.global_seq_lens,
                state.dcp_world_size,
                state.dcp_rank,
                state.cp_kv_cache_interleave_size,
            )
            # Write every copied field explicitly: at build time seq_lens
            # and cache_seq_lens_per_req alias one buffer view, but the
            # shadow verifier clones fields independently, so each named
            # field must be refreshed by name.
            metadata.seq_lens.copy_(local[: state.num_reqs])
            if (
                metadata.cache_seq_lens_per_req.data_ptr()
                != metadata.seq_lens.data_ptr()
            ):
                metadata.cache_seq_lens_per_req.copy_(
                    local[: state.num_reqs]
                )
            if (
                metadata.cache_seq_lens_per_token.data_ptr()
                != metadata.seq_lens.data_ptr()
            ):
                metadata.cache_seq_lens_per_token.copy_(
                    local[: state.num_tokens]
                )
        else:
            # DCP1: the rebuild's cache fields are the CLAMPED cm.seq_lens
            # ([DRAFT-INPUT RECOVERY]: pad rows 0 -> 1), so mirror the
            # clamp. The copies are identity no-ops in fused mode; in the
            # shadow sandbox they refresh the detached clones. (Under DCP1
            # the fused path is disabled -- see the supports property -- so
            # this branch only runs in the shadow sandbox and tests.)
            clamped = state.global_seq_lens.clamp_min(1)
            if metadata.seq_lens is not state.global_seq_lens:
                metadata.seq_lens.copy_(clamped)
            if (
                metadata.cache_seq_lens_per_req.data_ptr()
                != metadata.seq_lens.data_ptr()
            ):
                metadata.cache_seq_lens_per_req.copy_(clamped)
            if (
                metadata.cache_seq_lens_per_token.data_ptr()
                != metadata.seq_lens.data_ptr()
            ):
                metadata.cache_seq_lens_per_token.copy_(
                    clamped[: state.num_tokens]
                )


    def rebind_draft_decode_live_seq_lens(self, live_seq_lens: torch.Tensor) -> None:
        """[FUSED-DRAFT-DECODE] Bind the hook's live seq-len source.

        Called once by the speculator at configure time with
        input_buffers.seq_lens (the persistent buffer update_draft_inputs
        advances in place), so the stash refs track every inter-step
        advance by themselves (no per-step rebind).
        """
        self._draft_decode_live_seq_lens = live_seq_lens
...
class B12xMLASparseImpl(MLAAttentionImpl[B12xMLASparseMetadata]):
    ...
    def __init__(
        ...
        **mla_args,
        ...
        if indexer is not None:
            ...
            indexer_outputs_physical = not self.indexer_outputs_logical
        ...
        _kv_nope_fp8 = bool(
            ...
            and self.q_head_dim == _NOPE_Q_HEAD_DIM
        )
        if _kv_nope_fp8 and not _b12x_fp8_kv_enabled():
            # supports_combination refuses this combination with the gate
            # off; reaching this point means a selection-path bug. Fail
            # rather than size a 656-strided pool for a 528 record.
            raise RuntimeError(
                "B12X_MLA_SPARSE fp8_ds_mla + head_size=512 requires "
                f"{_B12X_FP8_KV_ENV}=1 (supports_combination must refuse "
                "this combination when the gate is off)"
            )
        self._kv_nope_fp8 = _kv_nope_fp8
        if self.kv_cache_dtype == "nvfp4_ds_mla":
            ...
            if self._kv_nope_fp8:
                # The validated r7 528B record: 512B e4m3 latent + 16B
                # inline fp32 scales, NO rope tail (pooled_indexer.py:64
                # _MLA_RECORD_BYTES=528). The 656 default is the
                # rope-bearing V3.2/NSA shape (head 576 keeps it).
                self._kv_record_bytes = _GLM_NOPE_FP8_KV_RECORD_BYTES
            else:
                self._kv_record_bytes = 656
        else:
            self._kv_record_bytes = 656
        if self._kv_nope_fp8:
            # The staged 528B writer (G3): byte layout pinned by
            # test_fp8_native.py T9 (e4m3 latent + 4 fp32 group-128 scales,
            # records contiguous at stride 528).
            try:
                from b12x.attention._shared.mla.kv_cache import (
                    concat_and_cache_glm_nope_fp8_mla,
                )
            except ImportError as exc:
                raise RuntimeError(
                    "VLLM_B12X_FP8_KV=1 requires a B12X build with "
                    "concat_and_cache_glm_nope_fp8_mla package API support"
                ) from exc
            self._concat_and_cache_glm_nope_fp8_mla = (
                concat_and_cache_glm_nope_fp8_mla
            )
        logger.info(
    ...
    def do_kv_cache_update(
        ...
        """
        if self._kv_nope_fp8:
            # The staged 528B writer (G3): e4m3 latent + 4 fp32 group-128
            # inline scales, records contiguous at stride 528. No k_pe /
            # rope tail (qk_rope_head_dim == 0).
            if kv_cache.numel() == 0:
                return
            if kv_cache_dtype != "fp8_ds_mla":
                raise RuntimeError(
                    f"GLM_NOPE FP8 writer reached a non-fp8 cache: "
                    f"{kv_cache_dtype!r}"
                )
            self._concat_and_cache_glm_nope_fp8_mla(
                kv_c_normed,
    ...
    def _append_current_chunk_to_gathered(
        ...
        if self._kv_fp8_rope:
            ...
            # DCP gather path writes the SAME 528B fp8_ds_mla record (the
            # stock ops.concat_and_cache_mla below is the rope-bearing
            # writer and would corrupt a NoPE pool).
            self._concat_and_cache_glm_nope_fp8_mla(
                kv_c,
                gathered_buffer,
                slots,
