...
# [GUARD-WARMUP EXEMPTION] True while a warmup/compile-warmup forward is
# executing. Warmup batches are SYNTHETIC by design (deterministic spaced
# slot values against dummy cache pools; results discarded), so the
# VLLM_DEBUG_KDA_INPUTS guards must not validate them — they would kill
# the boot. Diagnostic guards read this lazily (one attr read when on).
