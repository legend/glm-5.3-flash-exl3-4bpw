...
def _causal_conv1d_update_kernel(
    ...
    if IS_SPEC_DECODING:
        ...
        # [Xid-31/43 fix] Bound the rolling offset to the conv-state width.
        # A spec row carrying more accepted tokens than the num_spec+1 grid
        # (a short extend the runner classifies as a decode-with-drafts row)
        # previously rolled the offset past the state's last dimension and
        # read/wrote out of bounds (the CUDA_LAUNCH_BLOCKING-pinned illegal
        # access at the conv state write). Offsets past the representable
        # window clamp to its edge, exactly like the recurrent kernel's
        # max_query_len bound and the conv's own max_query_len token cap.
        conv_state_token_offset = tl.minimum(
            conv_state_token_offset, state_len - KERNEL_WIDTH + 1
        )
