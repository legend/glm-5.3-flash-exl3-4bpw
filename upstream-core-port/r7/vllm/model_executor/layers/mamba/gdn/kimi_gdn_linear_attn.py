...
# [DIFF-DUMP] differential-harness gate for the KDA layer (marker file on the
# host-writable /cache volume; zero-cost when absent; skipped during capture
# and warmup). Mirrors the indexer.py harness so both stacks emit one tag set.
_DIFF_KDA_DUMP_ENABLED: bool | None = None


def _diff_kda_dump_enabled() -> bool:
    global _DIFF_KDA_DUMP_ENABLED
    if _DIFF_KDA_DUMP_ENABLED is None:
        try:
            import os as _os

            _DIFF_KDA_DUMP_ENABLED = _os.path.exists("/cache/.diff-dump")
        except Exception:
            _DIFF_KDA_DUMP_ENABLED = False
    if not _DIFF_KDA_DUMP_ENABLED:
        return False
    try:
...
_DIFF_KDA_CUR = _DiffKdaCur()


def _diff_kda_dump(prefix, conv_state, recurrent_state, m, prev_sig) -> None:
    with torch.no_grad():
        cs = conv_state.detach().float()
        rs = recurrent_state.detach().float()
        step = _DIFF_KDA_STEPS.get("kda", 0)
        _DIFF_KDA_STEPS["kda"] = step + 1
        sig = (
            tuple(conv_state.shape),
            tuple(conv_state.stride()),
            conv_state.data_ptr(),
            tuple(recurrent_state.shape),
            tuple(recurrent_state.stride()),
            recurrent_state.data_ptr(),
        )
        if step >= 8 and getattr(_DIFF_KDA_CUR, "sig", None) == (prefix, sig):
            return
        _DIFF_KDA_CUR.sig = (prefix, sig)
        print(
            f"[DIFF-DUMP] KDA-STATE step={step} layer={prefix} "
            f"conv shape={sig[0]} stride={sig[1]} ptr={sig[2] % 1000000007} "
            f"sum={cs.sum().item():.4f} absmean={cs.abs().mean().item():.6f} | "
            f"rec shape={sig[3]} stride={sig[4]} ptr={sig[5] % 1000000007} "
            f"sum={rs.sum().item():.4f} absmean={rs.abs().mean().item():.6f}",
            flush=True,
        )
        del prev_sig
...
def _diff_fwd_idx_check(prefix, mixed_qkv, spec_token_indx, non_spec_token_indx):
    """[DIFF-DUMP] GDN-IDX-FWD: forward-site check of the token index tensors
    against the projected qkv rows (mixed_qkv.size(0)). The build()-side probe
    compares against m.num_actual_tokens; if that one is clean but this one
    trips, the seam is between the metadata grid and the token tensor the
    layers actually see. Loud only on divergence; deduped by signature."""
    with torch.no_grad():
        rows = int(mixed_qkv.size(0))
        parts = [f"layer={prefix}", f"rows={rows}"]
        bad = False
        for nm, t in (("ns_idx", non_spec_token_indx), ("spec_idx", spec_token_indx)):
            if t is None or t.numel() == 0:
                parts.append(f"{nm}=-")
                continue
            tmax = int(t.max().item())
            tmin = int(t.min().item())
            parts.append(f"{nm} len={t.numel()} [{tmin},{tmax}]")
            if tmax >= rows or tmin < 0 or t.numel() > rows:
                bad = True
        sig = (prefix, rows, bad)
        if bad or _DIFF_FWD_SIG.get("k", None) != sig:
            _DIFF_FWD_SIG["k"] = sig
            print(
                "[DIFF-DUMP] GDN-IDX-FWD" + ("-OOB " if bad else " ")
                + " ".join(parts),
                flush=True,
            )
...
def _kda_input_debug_enabled() -> bool:
    """VLLM_DEBUG_KDA_INPUTS=1 enables host-side input validation at the
    KDA prefill/recurrent call sites. Zero cost when off (cached env read);
    each check intentionally device-syncs, so it is diagnostics-only.

    CUDA graph capture forbids device<->host copies entirely, and replayed
    steps would pay a sync per launch, so the guard is skipped whenever the
    current stream is capturing (the same `torch.cuda.is_current_stream_capturing`
    convention the fork uses elsewhere)."""
    global _KDA_INPUT_DEBUG
    if _KDA_INPUT_DEBUG is None:
        _KDA_INPUT_DEBUG = bool(int(os.environ.get("VLLM_DEBUG_KDA_INPUTS", "0")))
    ...
    try:
        ...
        # Non-CUDA (or capture-unaware) torch: nothing to skip.
        pass
...
def _kda_input_violation(
    site: str,
    message: str,
    m: "GDNAttentionMetadata | None" = None,
    extra: dict[str, object] | None = None,
) -> None:
    """Dump every relevant tensor, then raise a distinctive error."""
    lines = [f"[KDA-INPUT-VIOLATION] site={site}: {message}"]
    if m is not None:
        lines.append(f"num_reqs={m.num_reqs} num_prefills={m.num_prefills} "
                     f"num_decodes={m.num_decodes} "
                     f"num_spec_decodes={m.num_spec_decodes}")
        if m.seq_lens is not None:
            lines.append(f"seq_lens={m.seq_lens.tolist()}")
        bt = getattr(m, "block_table_tensor", None)
        if bt is not None:
            # Per-request mamba state block ids for this group (all rows).
            lines.append(f"mamba_state_block_table={bt.tolist()}")
        if getattr(m, "spec_state_indices_tensor", None) is not None:
            lines.append(
                "spec_state_indices="
                f"{m.spec_state_indices_tensor.tolist()}"
            )
        if getattr(m, "prefill_state_indices", None) is not None:
            lines.append(
                f"prefill_state_indices={m.prefill_state_indices.tolist()}"
            )
        if getattr(m, "has_initial_state", None) is not None:
            lines.append(f"has_initial_state={m.has_initial_state.tolist()}")
        if getattr(m, "num_accepted_tokens", None) is not None:
            lines.append(
                f"num_accepted_tokens={m.num_accepted_tokens.tolist()}"
            )
...
def _kda_debug_check_index_tensor(
    site: str,
    name: str,
    ...
    """Assert every state-index value is a valid pool row."""
    if indices.numel() == 0:
        return
    idx = indices.to(torch.int64).cpu()
    bad = (idx < 0) | (idx >= num_blocks)
    if bool(bad.any()):
        rows, cols = (bad.nonzero(as_tuple=False)[:8]).tolist(), None
        flat_bad = idx[bad][:16].tolist()
        _kda_input_violation(
            site,
            f"{name}: values {flat_bad} out of bounds for state pool "
            f"num_blocks={num_blocks}; first bad positions={rows}",
            m=m,
            extra={**(extra or {}), f"{name}_full": idx.tolist()},
        )


def _kda_debug_check_cu_seqlens(
    site: str,
    cu_seqlens: torch.Tensor,
    num_tokens: int,
    m: "GDNAttentionMetadata | None" = None,
    ...
    vals = cu_seqlens.to(torch.int64).cpu().tolist()
    if vals[0] != 0 or any(
        b < a for a, b in zip(vals, vals[1:])
    ) or vals[-1] > num_tokens:
        _kda_input_violation(
            site,
            f"cu_seqlens {vals} not monotonic in [0, {num_tokens}]",
            m=m,
            extra=extra,
        )


def _kda_debug_validate_prefill(
    ...
    site = "chunk_kda_with_fused_gate_prefill"
    T = int(q.shape[1])
    _kda_debug_check_cu_seqlens(
        site, cu_seqlens, T, m, {"T": T, "state_shape": tuple(state.shape)}
    )
    _kda_debug_check_index_tensor(
        site, "prefill_state_indices", state_indices, int(state.shape[0]), m
    )
    N = int(cu_seqlens.numel()) - 1
    if chunk_indices is not None and chunk_indices.numel() > 0:
        ci = chunk_indices.cpu()
        bad = (ci[:, 0] < 0) | (ci[:, 0] >= N)
        if bool(bad.any()):
            _kda_input_violation(
                site,
                f"chunk_indices seq ids {ci[bad][:16].tolist()} out of "
                f"bounds for N={N}",
                m=m,
                extra={"chunk_indices": ci.tolist()},
            )
    if chunk_offsets is not None and chunk_offsets.numel() > 0:
        co = chunk_offsets.cpu()
        if bool((co < 0).any()) or int(co.max()) > max(T, 1):
            _kda_input_violation(
                site,
                f"chunk_offsets {co.tolist()} out of bounds",
                m=m,
                extra={"T": T},
            )


def _kda_debug_validate_prefill_scatter(
    state_indices: torch.Tensor,
    state_rows: int,
    ...
    """[PREFILL-STATE-SCATTER guard] The prefill branch's cache init scatter
    (`recurrent_state[prefill_state_indices] = last_recurrent_state`) had no
    pool-bound check; an out-of-pool index trips ATen's IndexKernel assert
    ('-sizes[i] <= index && index < sizes[i]') — the blocked capture of the
    same fault class. Dumps the offending VALUES (block-id scale = an
    over-max-context row; token-scale = a grid/table mix; negative = a
    null/freed stamp), the full indices, the mamba block table rows, the
    prefill grid, and the batch composition.
    """
    if state_indices is None or state_indices.numel() == 0:
        return
    idx = state_indices.to(torch.int64).cpu()
    bad = (idx < 0) | (idx >= state_rows)
    if bool(bad.any()):
        _kda_input_violation(
            "prefill_state_scatter",
            f"state indices {idx[bad][:16].tolist()} out of bounds for the "
            f"recurrent state pool rows={state_rows}",
            m=m,
            extra={
                "prefill_state_indices_full": idx.tolist()[:64],
                "prefill_query_start_loc": prefill_query_start_loc.to(
                    torch.int64
                ).cpu().tolist()[:64],
                "last_recurrent_state_shape": tuple(last_recurrent_state.shape),
            },
        )
    # Shape consistency: the scatter assigns len(indices) rows of state.
    if (
        last_recurrent_state.ndim >= 1
        and int(last_recurrent_state.shape[0]) != int(idx.numel())
    ):
        _kda_input_violation(
            "prefill_state_scatter",
            f"last_recurrent_state has {int(last_recurrent_state.shape[0])} "
            f"rows but prefill_state_indices has {int(idx.numel())} entries",
            m=m,
            extra={"prefill_state_indices_full": idx.tolist()[:64]},
        )


def _kda_debug_validate_spec(
    m: "GDNAttentionMetadata",
    cu_seqlens: torch.Tensor,
    state_indices: torch.Tensor,
    ...
    site = "fused_recurrent_kda_spec"
    T = int(q.shape[1])
    next_n = int(state_indices.shape[1]) if state_indices.ndim == 2 else 1
    _kda_debug_check_cu_seqlens(
        site, cu_seqlens, T, m, {"T": T, "next_n": next_n}
    )
    _kda_debug_check_index_tensor(
        site, "spec_state_indices", state_indices, int(state.shape[0]), m
    )
    if num_accepted_tokens is not None and num_accepted_tokens.numel() > 0:
        na = num_accepted_tokens.cpu()
        if bool((na < 0).any()) or bool((na > next_n).any()):
            _kda_input_violation(
                site,
                f"num_accepted_tokens {na.tolist()} out of bounds "
                f"[0, {next_n}]",
                m=m,
            )
...
def _kda_recompute_chunk_metadata(
    prefill_query_start_loc: torch.Tensor,
) -> tuple[torch.Tensor, torch.Tensor]:
    """Recompute the fla-layout chunk tables from the rebased prefill grid.

    Fallback for a stale metadata delivery; one small host sync.
    """
    chunk_indices = prepare_chunk_indices(prefill_query_start_loc, FLA_CHUNK_SIZE)
    chunk_offsets = prepare_chunk_offsets(prefill_query_start_loc, FLA_CHUNK_SIZE)
    return chunk_indices, chunk_offsets


def _kda_valid_chunk_metadata(
    chunk_indices: torch.Tensor | None,
    chunk_offsets: torch.Tensor | None,
    prefill_query_start_loc: torch.Tensor,
) -> tuple[torch.Tensor | None, torch.Tensor | None]:
    """Validate the carried chunk metadata against the current grid.

    Returns the metadata unchanged when it is valid for this grid, else
    (None, None) so the caller falls back to a recompute. Validation needs
    host values, so it is only sound off the capture path; the same small
    host sync the fallback would pay is paid here instead, and only on
    prefill steps.
    """
    ...
    try:
        if torch.cuda.is_current_stream_capturing():
            # Cannot host-validate during capture: the caller is a capture
            # step with freshly built metadata (the runner builds metadata
            # for every capture), so pass it through unchanged.
            return chunk_indices, chunk_offsets
    except Exception:
        pass
    try:
        cu = prefill_query_start_loc.to(torch.int64).cpu().tolist()
        n_prefills = len(cu) - 1
        total_tokens = cu[-1] if cu else 0
        ci = chunk_indices.to(torch.int64).cpu()
        co = chunk_offsets.to(torch.int64).cpu().tolist()
        valid = (
            n_prefills > 0
            and ci.numel() > 0
            and int(ci[:, 0].min()) >= 0
            and int(ci[:, 0].max()) < n_prefills
            and all(0 <= o <= total_tokens for o in co)
        )
        if valid:
            return chunk_indices, chunk_offsets
        # [STALE-CHUNK diagnostics] The carried chunk metadata is invalid
        # for this grid: dump its CONTENT so the producer is identifiable —
        # consecutive block-id-scale runs point at a staged-write leak; raw
        # token-offset-scale values point at an upstream grid mix.
        logger.warning(
            "[STALE-CHUNK dump] invalid carried chunk metadata "
            "(n_prefills=%d, total_tokens=%d): chunk_indices=%s "
            "chunk_offsets=%s cu_seqlens=%s",
            n_prefills,
            total_tokens,
            ci[:64].tolist(),
            co[:64],
...
class KimiGatedDeltaNetAttention(GatedDeltaNetAttention):
    ...
    def __init__(
        ...
        if self.kda_prefill_backend == "flashkda":
            ...
            # Reserve the FlashKDA prefill workspace before memory profiling
            # (the r7 `WorkspaceManager.reserve_all` contract: persistent
            # workspaces must exist before profiling so the first prefill
            # never grows device memory mid-profile). Growing the workspace
            # slot at the first prefill instead allocates right at the
            # memory-profiling peak, which on a nearly-full card surfaces as
            # a raw cudaErrorMemoryAllocation (v84 profile_single_request_prefill
            # runs with ~1.7 GiB of headroom after weights+workspaces).
            ...
            if is_workspace_manager_initialized():
                ...
                if hasattr(manager, "reserve_all"):
                    manager.reserve_all(*self._flashkda_buffer_specs)
                else:
                    manager.get_simultaneous(*self._flashkda_buffer_specs)
    ...
    def _forward(
        ...
        # [STALE-GRID fix] Metadata reused across batches (the runner's
        # update_block_table fast path) can carry query grids rebased to a
        # PREVIOUS batch's global token offset — e.g. a spec grid of
        # [4535, 4536] reaching a batch whose spec row must be [0, 1] —
        # while the row boundaries (diffs) are still correct. The KDA
        # kernels index token rows with these grids, so a stale offset is
        # an out-of-bounds read/write (Xid 31/43). Rebase each grid to zero
        # (a pure tensor op, no device sync; a no-op for valid grids whose
        # first entry is already 0).
        ...
        # [STALE-GRID fix] same rebasing as the non-spec grid above.
        if spec_query_start_loc is not None:
            spec_query_start_loc = spec_query_start_loc - spec_query_start_loc[:1]
        ...
        # [DIFF-DUMP] KDA state checksums for the upstream-port bisect: the
        # conv/recurrent state VIEWS the engine binds (shape/stride/offset +
        # contents). Gate = /cache/.diff-dump marker (see indexer.py harness).
        if _diff_kda_dump_enabled():
            _diff_kda_dump(
                self.prefix,
                conv_state,
                recurrent_state,
                m,
                getattr(self, "_diff_kda_sig", None),
            )
        if _kda_input_debug_enabled():
            # [BIND-SHAPE dump] Dumps whenever the bound views CHANGE (the
            # earlier one-shot flag captured only the profiling warmup's
            # minimal 8-row bind and never the serving bind). The serving
            # rows must match the ids' pool (198): if the real init's
            # re-bind keeps the minimal rows, the conv reads past the 8-row
            # view at any id >= 8 — VIRT_READ with clean in-pool ids.
            sig = (
                tuple(conv_state.shape),
                tuple(conv_state.stride()),
                tuple(recurrent_state.shape),
            ...
            if getattr(self, "_kda_bind_sig", None) != sig:
                self._kda_bind_sig = sig
                logger.warning(
                    "[BIND-SHAPE dump] layer=%s conv_state=%s stride=%s "
                    "dtype=%s | recurrent_state=%s stride=%s dtype=%s",
                    self.prefix,
                    tuple(conv_state.shape),
                    tuple(conv_state.stride()),
                    conv_state.dtype,
        ...
        if (
            ...
            and _diff_kda_dump_enabled()
        ):
            _diff_fwd_idx_check(
                self.prefix, mixed_qkv, spec_token_indx, non_spec_token_indx
            )
        ...
        if spec_sequence_masks is not None:
            ...
            if use_b12x_kda:
                ...
                core_attn_out_spec, _ = fused_recurrent_kda(
                    ...
                    num_accepted_tokens=num_accepted_tokens,
                    out=spec_out,
                    # [APC-Xid31/43 fix] bound the spec loop to the
                    # num_spec+1-wide state-index grid; an overlong spec row
                    # (short extend scheduled with more draft tokens than
                    # the configured depth) must not run past it.
                    max_query_len=spec_max_query_len,
                )
        ...
        if mixed_qkv_ns is not None:
            ...
            if m.num_prefills > 0:
                ...
                if split_non_spec:
                    ...
                    # [STALE-GRID fix] rebase to zero like the spec grids.
                ...
                if self.kda_prefill_backend == "flashkda":
                    ...
                    # [STALE-CHUNK fix, validate-then-fallback] Consume the
                    # builder's chunk metadata unchanged when it is valid for
                    # the current grid (fresh builds — the overwhelmingly
                    # common case — stay bit-identical to the builder's
                    # precomputed tables, including any backend-specific
                    # layout). Only a stale delivery (metadata reused across
                    # batches: token-scale ids or out-of-grid offsets — the
                    # dump class) falls back to a recompute from the rebased
                    # grid, because a wrong-scale table cannot be rebased.
                    chunk_indices, chunk_offsets = _kda_valid_chunk_metadata(
                        m.chunk_indices,
                        m.chunk_offsets,
                        prefill_query_start_loc,
                    )
                    if chunk_indices is None:
                        chunk_indices, chunk_offsets = (
                            _kda_recompute_chunk_metadata(prefill_query_start_loc)
                        )
                    if _kda_input_debug_enabled():
                        _kda_debug_validate_prefill(
                            m=m,
                            cu_seqlens=prefill_query_start_loc,
                ...
                # Init cache
                if _kda_input_debug_enabled():
                    _kda_debug_validate_prefill_scatter(
                        prefill_state_indices,
