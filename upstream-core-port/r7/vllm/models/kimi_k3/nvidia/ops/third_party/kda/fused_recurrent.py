...
def fused_recurrent_kda_fwd_kernel(
    ...
    if MAX_QUERY_LEN > 0:
        initial_token = tl.minimum(initial_token, MAX_QUERY_LEN - 1)
    state_index = tl.load(state_indices + i_n * stride_indices_seq + initial_token).to(
        tl.int64
    )
    # [APC-Xid31/43 fix] Bound the per-row spec loop to the state-index
    # grid. A spec row carrying more tokens than num_spec + 1 (a short
    # extend the runner classifies as a decode-with-drafts row) previously
    # read ``state_indices`` columns past its width and stored the final
    # state at the garbage id read from there (VIRT_READ + wild VIRT_WRITE).
    # Mirrors causal_conv1d_update's max_query_len contract: tokens beyond
    # the representable grid are dropped exactly like the conv path.
    if MAX_QUERY_LEN > 0:
        sequence_length = tl.minimum(sequence_length, MAX_QUERY_LEN)
...
def fused_recurrent_kda_fwd(
    ...
    ``max_query_len`` bounds the per-row token loop in spec decoding: the
    spec state-index grid has ``num_spec + 1`` columns, so a row scheduled
    with more draft tokens than the configured depth must not run past it
    (its extra state columns do not exist). ``None``/0 disables the bound.
    """
    B, T, H, K = q.shape
    V = v.shape[-1]
    ...
    fused_recurrent_kda_fwd_kernel[grid](
        ...
        N=N,
        T=T,
        MAX_QUERY_LEN=int(max_query_len) if max_query_len else 0,
        H=H,
        K=K,
        V=V,
