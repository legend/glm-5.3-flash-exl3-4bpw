...
def _pack_dcp_partial_output(partial_output: torch.Tensor) -> torch.Tensor:
    """Return the DCP partial attention output in the reducer's tensor ABI.

    ``torch.ops._C.direct_dcp_a2a_lse_reduce`` requires the rank-local
    partial output [T, H, D] to have packed heads (stride(2) == 1,
    stride(1) == head_dim) and a token stride that is >= H*D and
    8-aligned (csrc .../dcp_direct_a2a_lse_reduce.cu: "partial output
    must have packed heads and an aligned token stride"). The v84 flow
    fed the combine the in-image b12x decode-plan buffer, which is
    planned token-major whenever dcp_world_size > 1 (head-major output
    is planned only for dcp_world_size == 1); the b12x extend-plan
    buffer, however, is always materialized head-major (stride
    (D, T_max*D, 1)). When the flow reaches the combine with an
    extend-path output, pack it into a contiguous buffer; layouts that
    already satisfy the ABI (the token-major decode buffer and its
    head-sliced pad view) pass through with no copy.
    """
    heads, head_dim = partial_output.shape[1], partial_output.shape[2]
    token_stride = partial_output.stride(0)
    if (
        partial_output.stride(2) == 1
        and partial_output.stride(1) == head_dim
        and token_stride >= heads * head_dim
        and token_stride % 8 == 0
    ):
        return partial_output
    return partial_output.contiguous()
...
class MLAAttention(nn.Module, AttentionLayerBase):
    ...
    def __init__(
        ...
        **extra_impl_args,
        ...
        # [FORK-COMPAT] v84 in-image consumers (model_executor/warmup/
        # kernel_warmup.py `isinstance(module, MLAAttention) and
        # module.dcp_b12x`) read this attribute on every MLA warmup sweep.
        # The v84 fork computed it as `dcp_a2a and not use_pcp and
        # VLLM_USE_B12X_DCP_A2A and attn_backend == "B12X_MLA_SPARSE"`; this
        # upstream port's DCP path goes through MLADCPManager dispatch instead
        # of the fork's B12X PCIe A2A channel, so the stock upstream value is
        # always False (the fork's b12x all-gather warmup branch is inert).
        self.dcp_b12x: bool = False
    ...
    def forward_impl(
        ...
        if num_mqa_tokens > 0:
            # [FORK-COMPAT] the in-image B12xMLASparseImpl lacks
            # uses_full_ckv_dcp (an r7-native method); the fork's DCP path
            # computes the local projection unconditionally (v84 semantics:
            # full-CKV DCP off => local CKV plan).
            _uses_full = getattr(
                self.impl, "uses_full_ckv_dcp", None
            )
            full_ckv_dcp = (
                _uses_full(attn_metadata, num_mqa_tokens)
                if _uses_full is not None
                else False
            )
            ...
            if self.impl.dcp_world_size > 1 and not full_ckv_dcp:
                ...
                # [FORK-COMPAT] the in-image b12x extend plan materializes
                # its output buffer head-major while the direct DCP reducer
                # requires packed heads and an aligned token stride. Pack on
                # demand so the reducer sees exactly the v84-equivalent
                # token-major buffer regardless of which impl branch ran.
                attn_out = _pack_dcp_partial_output(attn_out)
...
def build_mla_chunked_context_metadata(
    *,
    ...
    starts = _flat_int32(starts_flat).to(device)
    cu_seq_lens = _flat_int32(cu_seq_lens_flat).to(device)
    cu_seqlens_q = _flat_int32(cu_seqlens_q_flat).to(device)
