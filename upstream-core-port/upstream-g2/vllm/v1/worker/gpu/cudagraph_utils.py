...
# [FORK-COMPAT] v84 capture-scope context var: in-image multi-stream custom
# ops (aux-stream CUDA-event pools in kimi_k3/glm5next/deepseek_v4 layers and
# v1/attention/ops) query this when the per-current-stream capture query is
# insufficient during graph capture. v84 wrapped the whole manager-level
# capture in the scope; upstream has no equivalent. In-image module, unmounted.
...
class CudaGraphManager:
    ...
    def capture(
        ...
        *,
        # [FORK-COMPAT] v84 semantic channel id: a stable distributed identity
        # for this graph owner, consumed by the in-image
        # parallel_state.graph_capture() for CA-capture resource bookkeeping.
        # Upstream dropped the concept; the in-image CudaGraphManager
        # subclasses that stay mounted (SpeculatorCudaGraphManager,
        # DFlashCudaGraphManager, DFlashContextCudaGraphManager) pass it to
        # super().capture(), so the staged base must accept it.
        ...
        Args:
            ...
            channel_id: Optional stable distributed identity for this graph
                owner ([FORK-COMPAT] v84; forwarded to graph_capture).
        """
        # [FORK-COMPAT] v84 capture context: mark Python execution as
        # belonging to a vLLM CUDA graph capture for the whole capture pass.
        # In-image multi-stream custom ops run on joined auxiliary streams
        # where CUDA's per-current-stream capture query is false even though
        # graph nodes retain their event handles; the scope makes their event
        # pools allocate private per-graph sets during capture instead of
        # reusing the eager default handles. Upstream has no equivalent.
        with (
            ...
            for mode in [CUDAGraphMode.PIECEWISE, CUDAGraphMode.FULL]:
                ...
                for desc in descs:
                    ...
                    if (
                        ...
                        # [FORK-COMPAT] v84 B12X prewarm: B12X kernels use
                        # caller-owned scratch views in the CuTe launcher
                        # contract. Re-warm the exact fresh state that FULL
                        # capture will use, so CUDA graph capture only
                        # records resolved launches. Dormant unless a
                        # VLLM_USE_B12X_* env is set (v84 semantics, in-image
                        # vllm.compilation.b12x_capture).
                        ...
                        with (
                            ...
                            # [FORK-COMPAT] v84 B12X guard: freeze kernel
                            # resolution during FULL capture so no lazy
                            # b12x kernel selection is recorded mid-graph
                            # (no-op unless VLLM_USE_B12X_* is set).
                            guard_b12x_kernel_resolution(
...
class ModelCudaGraphManager(CudaGraphManager):
    ...
    def capture(
        ...
        super().capture(
            create_forward_fn, progress_bar_desc, channel_id=channel_id
        )
...
def profile_cudagraph_memory(runner: "GPUModelRunner") -> int:
    ...
    try:
        ...
        torch.accelerator.synchronize()
        # [FORK-COMPAT] v84 CUDAGraphWrapper/BreakableCUDAGraphWrapper lack
        # upstream's reset_all_graphs (they only provide clear_all_graphs,
        # invoked just below); skip the deferred-destruction reset on forks.
        if hasattr(CUDAGraphWrapper, "reset_all_graphs"):
            CUDAGraphWrapper.reset_all_graphs()
        if hasattr(BreakableCUDAGraphWrapper, "reset_all_graphs"):
