...
class AutoRegressiveSpeculator(DraftModelSpeculator):
    ...
    def capture(self) -> None:
        ...
        self.prefill_cudagraph_manager.capture(
            ...
            # [FORK-COMPAT] the mounted SpeculatorCudaGraphManager (in-image
            # autoregressive/cudagraph_utils.py) declares channel_id as a
            # required keyword; upstream dropped the v84 capture-phase
            # argument, so use the v84-style stable identity without the
            # phase suffix (v84: f"vllm:draft:prefill:{capture_phase}").
            channel_id="vllm:draft:prefill",
        ...
        self.decode_cudagraph_manager.capture(
            ...
            self.attn_groups,
            self.kv_cache_config,
            # [FORK-COMPAT] see the prefill capture call above: the mounted
            # in-image SpeculatorCudaGraphManager requires channel_id
            # (v84: f"vllm:draft:decode:{capture_phase}").
            channel_id="vllm:draft:decode",
    ...
    def propose(
        ...
        # [FORK-COMPAT v84] DCP draft-prefill metadata rebuild. When the
        # draft has an attention layer in a dcp-replicated KV group (GLM-5.3
        # MTP kpool tail under --decode-context-parallel-size 2), the target
        # batch's metadata/slot mappings cannot be reused: the replicated
        # group's boundary state is global, not token-sharded. Rebuild the
        # draft's own prefill attention state (v84 propose() verbatim,
        # adapted to the overlay's _build_draft_attn_metadata signature —
        # self.draft_max_seq_len already carries v84's
        # max_seq_len_upper_bound). The FULL-graph branch keeps the v84
        # behavior of calling the rebuild for its side effects: it refreshes
        # the shared static buffers (block_tables slot mappings, dcp local
        # seq lens, draft builder buffers) that the captured graph reads.
        rebuild_prefill_attn_metadata = False  # [REVERT-20260904: crashed draft-prefill rebuild on kpool tail; acceptance is a contention artifact per AcceptFix. Revisit with the correct pool-granular slot mappings before re-enabling.
    ...
    def _run_model(
        ...
        with set_forward_context(
            ...
            # [BOUNDS SAFETY - Xid 31/43 class] The draft input ids feed the
            # embedding-table gather directly (mm branch below, or the model
            # forward in the text-only branch). A stale or torn id (rejected
            # slot reuse, host-source tear in the staging chain) would index
            # the table out of range and trip a device assert; clamping into
            # the vocab range degrades a bad id to a valid lookup instead of
            # a fault. In-place op, CUDA-graph-safe, runs for every draft
            # prefill/decode step through this chokepoint.
            self.input_buffers.input_ids[:num_tokens].clamp_(
                0, self.vocab_size - 1
            )
            inputs_embeds = None
