...
class RejectionSampler:
    def __init__(
        ...
        # [FORK-COMPAT] v84 SpeculativeConfig lacks enable_adaptive_verification
        self.enable_adaptive_verification = getattr(
            spec_config, "enable_adaptive_verification", False
        )
    ...
    def __call__(
        ...
        # [V84-RESTORE] the in-image rejection sampler masks padding rows to
        # the -1 placeholder so the kernel's is_valid_draft check terminates
        # verification there instead of "verifying" a garbage padded draft
        # token (reachable when the scheduler schedules fewer draft tokens
        # than the fixed depth, e.g. near max_model_len).
