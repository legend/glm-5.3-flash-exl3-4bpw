...
def _copy_mamba_state_block(
    ...
    block_table_widths_ptr,
    state_pool_num_blocks_ptr,
    align_clamp_ctr_ptr,
):
    ...
    # [XID-31/43 BARRIER] bounds-clamp every block-table access and state
    # write: a stale/garbage column or block id (async-scheduling staging
    # churn under concurrent chunked prefills + eviction) previously read
    # past the table row and wrote state at state_base + garbage_id * stride
    # - outside the pool - producing Xid 31 FAULT_PDE VIRT_WRITE on
    # unmapped pages, or silently corrupting adjacent tensors (the varying
    # consumer-side device asserts of the 2026-09-03 captures: run 1
    # non_spec_token_indx, run 2 idx_mapping). Same fix class as the v84
    # rev2 _mamba_select_block_table_kernel clamp. Each skip bumps the
    # align_clamp_hits counter so the harness probe names the first step.
    bt_width = tl.load(block_table_widths_ptr + group_idx).to(tl.int64)
    pool_num_blocks = tl.load(state_pool_num_blocks_ptr + state_idx).to(tl.int64)
    # Legitimate accepted-token biases are always >= 0; a garbage negative
    # bias would drive copy sizes/offsets below the block base.
    token_bias = tl.maximum(token_bias, 0)
    if dst_col < 0 or dst_col >= bt_width:
        if tile_idx == 0:
            tl.atomic_add(align_clamp_ctr_ptr, 1)
        return
    dest_block_id = tl.load(block_table_base + dst_col).to(tl.int64)
    if dest_block_id < 0 or dest_block_id >= pool_num_blocks:
        if tile_idx == 0:
            tl.atomic_add(align_clamp_ctr_ptr, 1)
        return
    ...
    if CONV_STATE_DIM_FIRST and is_conv_state:
        ...
        # [XID-31/43 BARRIER] src column/id must stay in the row/pool.
        if src_col < 0 or src_col >= bt_width:
            if tile_idx == 0:
                tl.atomic_add(align_clamp_ctr_ptr, 1)
            return
        src_block_id = tl.load(block_table_base + src_col).to(tl.int64)
        if src_block_id < 0 or src_block_id >= pool_num_blocks:
            if tile_idx == 0:
                tl.atomic_add(align_clamp_ctr_ptr, 1)
            return
    ...
    # [XID-31/43 BARRIER] src column/id must stay in the row/pool.
    src_read_col = src_col.to(tl.int64) + token_bias.to(tl.int64)
    if src_read_col < 0 or src_read_col >= bt_width:
        if tile_idx == 0:
            tl.atomic_add(align_clamp_ctr_ptr, 1)
        return
    actual_src_block_id = tl.load(block_table_base + src_read_col).to(tl.int64)
    if actual_src_block_id < 0 or actual_src_block_id >= pool_num_blocks:
        if tile_idx == 0:
            tl.atomic_add(align_clamp_ctr_ptr, 1)
        return
...
def postprocess_mamba_fused_kernel(
    ...
    num_reqs,
    # [XID-31/43 BARRIER] bounds metadata (required, no default: every
    # launcher passes them; see _copy_mamba_state_block)
    block_table_widths_ptr,
    state_pool_num_blocks_ptr,
    align_clamp_ctr_ptr,
...
def preprocess_mamba_align_fused_kernel(
    ...
    # [XID-31/43 BARRIER] clamp the post-advance running-block column to the
    # block-table row: garbage computed/token counts (stale staging under
    # async scheduling + eviction churn) previously stored columns past the
    # row, which the copy kernels would then read out of bounds. -1 is the
    # "fresh request" sentinel.
    new_state_idx = tl.maximum(tl.minimum(new_state_idx, max_state_col), -1)
...
def precopy_mamba_align_fused_kernel(
    ...
    num_reqs,
    # [XID-31/43 BARRIER] bounds metadata (required, no default)
    block_table_widths_ptr,
    state_pool_num_blocks_ptr,
    align_clamp_ctr_ptr,
    COPY_BLOCK_SIZE: tl.constexpr,
...
class MambaSpecDecodeGPUContext:
    ...
    # [XID-31/43 BARRIER] per-group block-table row width + per-state pool
    # size, consumed by the fused copy kernels' bounds clamps; the counter
    # records every clamped/skipped copy for the harness probe.
    block_table_widths: torch.Tensor
    state_pool_num_blocks: torch.Tensor
    align_clamp_hits: torch.Tensor
    ...
    def create(
        ...
        return cls(
            ...
            block_table_widths=torch.zeros(
                len(mamba_group_ids), dtype=torch.int32, device=device
            ),
            state_pool_num_blocks=torch.zeros(
                total_states, dtype=torch.int64, device=device
            ),
            align_clamp_hits=torch.zeros(1, dtype=torch.int32, device=device),
    ...
    def _populate_metadata(
        ...
        for group_local_idx, layer_group in enumerate(self.layer_groups):
            for layer_name, layer_spec in layer_group.layer_specs.items():
                ...
                for state, copy_func in zip(
                    ...
                    # Pool bound for the copy-kernel id clamps
                    self.state_pool_num_blocks[idx] = state.size(0)
                    # Base address
        ...
        for i, bt in enumerate(block_tables):
            ...
            self.block_table_widths[i] = bt.size(1)
    ...
    def run_fused_postprocess(
        ...
        postprocess_mamba_fused_kernel[grid](
            ...
            num_reqs,
            block_table_widths_ptr=self.block_table_widths,
            state_pool_num_blocks_ptr=self.state_pool_num_blocks,
            align_clamp_ctr_ptr=self.align_clamp_hits,
