...
# ---------------------------------------------------------------------------
# [v84-fork compat] Layout API kept for the un-ported fork backend files
# (hpc_attn, flashinfer*, tokenspeed_mla, cpu_attn, b12x_attn,
# v1/attention/selector.py, kv connectors, minimax sparse attention), which
# import these symbols from this module. Upstream replaced the
# override/global-variable scheme with ``resolve_kv_cache_layout`` /
# ``record_kv_cache_layout`` on CacheConfig; the shims below bridge the two:
# the override still wins, then the layout recorded by the engine core
# (mapped back to the fork's NHD/HND vocabulary), then the legacy
# env/connector/default chain.
# ---------------------------------------------------------------------------
KVCacheLayoutType = Literal["NHD", "HND"]

_KV_CACHE_LAYOUT_OVERRIDE: KVCacheLayoutType | None = None

# Legacy fork name -> upstream enum member and back.
_LAYOUT_LEGACY_TO_ENUM = {"NHD": KVCacheLayout.LBNHC, "HND": KVCacheLayout.LBHNC}
_LAYOUT_ENUM_TO_LEGACY = {
...
def get_kv_cache_layout() -> KVCacheLayoutType:
    """Resolved layout in the fork's NHD/HND vocabulary.

    Priority: ``set_kv_cache_layout`` override, the layout resolved by the
    engine core (``CacheConfig.kv_cache_layout``, set via
    ``record_kv_cache_layout``; LBNHC/LBHNC map back to NHD/HND), the
    ``VLLM_KV_CACHE_LAYOUT`` env var, the KV connector preference, then the
    default (NHD).
    """
    ...
    if vllm_config is not None:
        recorded = vllm_config.cache_config.kv_cache_layout
        if recorded is not None:
            layout = _layout_from_name(recorded)
            return _LAYOUT_ENUM_TO_LEGACY.get(layout, layout.name)

    cache_layout = envs.VLLM_KV_CACHE_LAYOUT
    if cache_layout is None:
        return get_kv_connector_cache_layout() or "NHD"
...
def _get_kv_connector_cache_layout_pref(vllm_config: VllmConfig):
    """[FORK-COMPAT] Connector layout preference.

    Upstream passes the config explicitly; the fork-side
    ``kv_connector/utils.get_kv_connector_cache_layout`` (kept in the image,
    unmounted) predates that API: it takes no arguments and reads the
    current global vllm config itself. Call whichever signature exists.
    """
    import inspect

    if inspect.signature(get_kv_connector_cache_layout).parameters:
...
def resolve_kv_cache_layout(
    ...
    if (requested := envs.VLLM_KV_CACHE_LAYOUT) is not None:
        ...
        connector := _get_kv_connector_cache_layout_pref(vllm_config)
    ) is not None:
...
def compute_causal_conv1d_metadata(
    ...
    for BLOCK_M in [8]:  # cover all BLOCK_M values
        ...
        # [H2D LIFETIME FIX] mlist/offsetlist are pinned locals that die at
        # function return; their non_blocking copies could still be in
        # flight (same class as async_copy_to_gpu). Stage synchronously.
...
def mamba_get_block_table_tensor(
    ...
    if mamba_cache_mode in ("all", "none"):
        ...
        # [XID-31/43 BARRIER] the v84 rev2 "full rows" clamp: `seq_lens` can
        # be a CPU upper bound that overshoots the allocated row (shrunken or
        # full rows), driving the gather window past the row end. Clamp the
        # window start to the row so the gather reads in-bounds (stale but
        # pool-valid ids) instead of asserting / reading garbage.
        max_start = max(
            0, block_table.size(1) - (1 + kv_cache_spec.num_speculative_blocks)
        )
