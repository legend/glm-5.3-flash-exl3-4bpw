...
class EngineCore:
    ...
    def __init__(
        ...
        # Local patch (Wynn's box, 2026-08-29): prefill-throttle cadence state.
        # Base-class defaults so _should_throttle_prefills is safe on the
        # non-DP engine; DPEngineCoreProc.__init__ overwrites both with its
        # own values (same attrs, identical semantics). Default interval of 1
        # disables throttling unless --prefill-schedule-interval is set.
        ...
        # Setup Model.
        # [UPSTREAM-PORT] Set the KV cache layout BEFORE spawning workers so
        # every worker's config copy carries it (cache.py treats an explicit
        # set as final — "unless explicitly set by the user"). BLHNC is the
        # layout verified for this deployment (DeepseekV32IndexerBackend +
        # B12X_MLA_SPARSE; diag/glm5-group-repro.py PASS).
    ...
    def _should_throttle_prefills(self) -> bool:
        """Whether to defer new prefills this step (prefill/decode interleave).

        Local patch (Wynn's box, 2026-08-29): previously hard-returned False —
        prefill throttling was DP-only. Now honors the throttle in the non-DP
    ...
    def step(self) -> tuple[dict[int, EngineCoreOutputs], bool]:
        ...
        if not self.scheduler.has_requests():
            # Idle: reset the cadence counter so the first step after a fresh
            # wave sees 0 and admits prefills immediately (DP semantics).
            self.step_counter = 0
            return {}, False
        ...
        self.step_counter += 1  # local patch: advance prefill-throttle cadence
