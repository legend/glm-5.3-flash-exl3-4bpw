...
def _storage_check_enabled() -> bool:
    """Gate for _validate_kv_cache_view_storage. VLLM_B12X_STORAGE_CHECK=1
    keeps the per-call storage-extent validation on; by default (unset/"0")
    only the FIRST call per process validates (fail-closed at warmup / CUDA
    capture, so a bad pool view still surfaces cleanly there) and later
    calls skip it: the check is pure-Python tensor attribute arithmetic on
    every launcher call (per eager prefill chunk), which added up in the
    decode/prefill hot path."""
...
def _validate_kv_cache_view_storage(cache: torch.Tensor, name: str) -> None:
    ...
    The launchers derive page_block_size / stride_kv_block from the VIEW's own
    shape/strides, so a view whose claimed extent exceeds its backing storage
    makes the in-kernel gather (block = idx // pbs; off = block*stride +
    local*_IOS) walk to addresses that are in-view but outside the allocation:
    an in-kernel CUDA_ERROR_ILLEGAL_ADDRESS that surfaces asynchronously at the
    NEXT kernel launch -- an unattributable Xid with "every API input in
    bounds" (the bounds were checked against the view's shape, not its
    storage). Contiguous views bypass the shared-API packed-view validation,
    so check here too: the view's max reachable element offset must stay
    inside its storage. == b12x.integration.mla _validate_tensor_storage_bounds.
    """
    ...
    if min_offset < 0 or max_offset >= storage_elems:
        raise ValueError(
            ...
            "KV view emission; e.g. a full-pool-shaped view based one "
            "attention page apart per layer)"
        )
...
def run_unified_decode(
    *,
    ...
    # every downstream in-view walk (pbs / stride / block decomposition) read
    # out of the allocation -- an async CUDA_ERROR_ILLEGAL_ADDRESS at the next
    # launch instead of an attributable error here.
    _validate_kv_cache_view_storage(swa_k_cache, "sparse MLA decode swa_k_cache")
    if has_extra:
        _validate_kv_cache_view_storage(
            indexed_k_cache, "sparse MLA decode indexed_k_cache"
        )

    if swa_k_cache.ndim >= 3:
        ...
        # of a scratch-plan / manager-block page scalar. The caller's
        # ``swa_page_size`` stays authoritative only for flat 1-D caches.
        # [WARMUP GUARD] The profiling warmup binds a minimal dummy cache
        # (e.g. (1,1,288)) whose shape[1] is NOT the kernel page size; deriving
        # pbs from it turns small warmup indices into wild block indices
        # (block = idx // 1 = idx). Fall back to the caller's page scalar
