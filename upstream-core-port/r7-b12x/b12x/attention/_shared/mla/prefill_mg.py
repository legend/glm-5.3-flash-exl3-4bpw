...
def _cache_block_stride_bytes(
    ...
    *,
    ...
    if is_glm:
        ...
        # ARBITRARY_FP32), 432B/368B/304B/288B (NVFP4), 528B (GLM_NOPE FP8 /
        # fp8_ds_mla). ``record_bytes`` comes from traits.kv_gmem_stride and
        # is ALWAYS threaded for GLM callers (the sole call site passes
        # traits.kv_gmem_stride), so the 656 default below only covers
        # rope-bearing shapes; a NoPE FP8 record can never hit it.
