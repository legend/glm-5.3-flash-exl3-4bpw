...
# GLM_NOPE FP8 (fp8_ds_mla -- the validated r7 528B ABI, pooled_indexer.py:64
# _MLA_RECORD_BYTES=528 / REFERENCE-VALIDATION.md §1.1): the record is the
# STRICT [0, 528) prefix of the 656B rope-bearing record above -- 512B e4m3
# latent + 16B inline fp32 scales -- with NO rope tail (qk_rope_head_dim == 0).
# The width authority (opt-work/fp8-native/staged/fp8_kv_record_abi.py)
# derives it; this constant is the kernel-side mirror, kept in lockstep by
# test_fp8_native.py.
...
def _glm_fp8_no_rope_gmem_stride(kv_gmem_stride: int) -> int:
    """Resolve the GLM_NOPE FP8 per-token record stride at TRACE time.

    Mirrors ``_nvfp4_no_rope_gmem_stride``: an explicitly threaded
    ``traits.kv_gmem_stride`` is AUTHORITATIVE (and validated == 528), the
    derived default is the 528B rope-less fp8_ds_mla record. Kept separate
    from the NVFP4 helper so an accidental NVFP4/FP8 trait mix-up fails
    loudly here instead of silently staging a wrong-width row.
    """
    if kv_gmem_stride:
        if int(kv_gmem_stride) != _GLM_NOPE_GMEM_STRIDE:
            raise ValueError(
                "GLM_NOPE FP8 kv_gmem_stride must be 528 (the rope-less "
                f"fp8_ds_mla record); got {int(kv_gmem_stride)}"
            )
        return int(kv_gmem_stride)
    return _GLM_NOPE_GMEM_STRIDE
...
def io_issue_gather(
    ...
    *,
    ...
    bulk_tx_bytes: cutlass.Constexpr,  # BI*(448+128)=36864 DSV4 / BI*(528+128)=41984 GLM
                       # / BI*528=33792 GLM_NOPE FP8 (no rope tail)
    ...
    group ([0, io_threads)). Defaulting to 32 keeps the decode PTX byte-identical.
      ...
      4. cp.async.bulk each entry; DSV4 issues separate NoPE (448B) and RoPE
         ...
         latent copy -- its record has no rope tail and its ``kv_rope``
         smem region is 0 bytes wide, so the second copy is const_expr-elided
         rather than aimed at a zero-extent region. The latent bulk width
         follows the dtype: 288B (NVFP4) or 528B (FP8, inline fp32 scales).
    ...
    if cutlass.const_expr(scale_format == 0):
        ...
        if cutlass.const_expr(_no_rope):
            # GLM_NOPE FP8 (fp8_ds_mla): the whole record IS the 528B
            # latent+inline-scales prefix -- 512B e4m3 + 16B inline fp32 --
            # with NO rope tail. The rope bulk below is const_expr-elided by
            # the ``not _no_rope`` gates, and _ROPE / _ROPE_SRC are
            # deliberately left UNBOUND: a regression that resurrected the
            # rope copy would fail LOUDLY at trace time (NameError) instead
            # of reading a phantom 128B tail past every 528B record. Same
            # pattern as the NVFP4 NoPE arm above.
            _IOS = Int64(_glm_fp8_no_rope_gmem_stride(kv_gmem_stride))
        else:
    ...
    if cutlass.const_expr(packed_glm and _no_rope):
        # The native H8 packed path stages one contiguous 656B record and is
        # GLM_NSA-only (kernel.py native_glm_h8 requires ModelType.GLM_NSA);
        # a GLM_NOPE FP8 cache must take the generic 528B arm. Fail at trace
        # time rather than staging 656B from a 528B record.
        raise AssertionError(
            "packed GLM H8 staging requires the 656B rope-bearing record; "
            "GLM_NOPE FP8 uses the generic 528B arm"
        )
