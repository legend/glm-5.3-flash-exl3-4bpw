    ...
    *,
    ...
    if model_type in (ModelType.GLM_NSA, ModelType.GLM_NOPE):
        ...
        if cache.ndim >= 3:
            ...
            # rec], stride(0) == the per-layer block stride -- the
            # 64-token / 18432-byte kernel-page contract), i.e. the same
            # source the walk must match. Re-deriving a scalar
            # ``page_size * rec`` product instead trusts a scratch-plan /
            # manager-block / bucket-level number that can disagree with
            # the emitted view geometry and silently stride the walk at
            # the wrong spec level (manager block vs kernel page). For a
            # contiguous view this is byte-identical to the old scalar
            # product whenever the caller's page scalar already matches
            # the view; when it does not, the view wins.
            stride = int(cache.stride(0)) * int(cache.element_size())
            payload = int(cache.shape[1]) * rec
            if stride < payload:
                raise ValueError(
                    f"SM120 sparse MLA cache block stride {stride} is smaller "
                    f"than page payload {payload}"
                )
            return stride
        # Flat / 2-D GLM caches carry no block axis: the dense scalar
        # product is the only geometry available.
        return int(page_size) * rec
    # DSV4 keeps the historical scalar-product contract (validated upstream).
...
_STORAGE_CHECK_ENABLED: bool | None = None
_STORAGE_CHECK_RAN_ONCE: bool = False


def _storage_check_enabled() -> bool:
    """Gate for _validate_kv_cache_view_storage (see kernel.py twin):
    VLLM_B12X_STORAGE_CHECK=1 keeps the per-call storage-extent validation
    on; by default only the FIRST call per process validates (fail-closed
    at warmup / capture) and later calls skip it -- the check was pure-Python
    tensor attribute arithmetic on every eager prefill chunk."""
    global _STORAGE_CHECK_ENABLED, _STORAGE_CHECK_RAN_ONCE
    if _STORAGE_CHECK_ENABLED is None:
        import os

        _STORAGE_CHECK_ENABLED = (
            os.environ.get("VLLM_B12X_STORAGE_CHECK", "0") == "1"
        )
    if _STORAGE_CHECK_ENABLED:
        return True
    if not _STORAGE_CHECK_RAN_ONCE:
        _STORAGE_CHECK_RAN_ONCE = True
        return True
    return False


def _validate_kv_cache_view_storage(cache: torch.Tensor, name: str) -> None:
    """FAIL-CLOSED storage-bounds check for a per-layer KV pool view.

    The dispatcher derives page_block_size / stride_kv_block from the VIEW's
    own shape/strides, so a view whose claimed extent exceeds its backing
    storage makes the MG gather walk to in-view but out-of-allocation
    addresses (an async CUDA_ERROR_ILLEGAL_ADDRESS at the next launch).
    Contiguous views bypass the shared-API packed-view validation, so check
    here too. == kernel.py _validate_kv_cache_view_storage /
    b12x.integration.mla _validate_tensor_storage_bounds.
    """
    if not _storage_check_enabled():
        return
    if cache.numel() == 0:
        return
    try:
        ...
        # Meta / fake tensors carry no real storage: nothing to validate.
        return
    min_offset = int(cache.storage_offset())
    max_offset = int(cache.storage_offset())
    for size, stride in zip(cache.shape, cache.stride(), strict=True):
        ...
        if extent >= 0:
            ...
            min_offset += extent
    storage_elems = storage.nbytes() // cache.element_size()
    if min_offset < 0 or max_offset >= storage_elems:
        raise ValueError(
            f"{name} view is out of storage bounds: shape={tuple(cache.shape)} "
            f"stride={tuple(cache.stride())} "
            f"storage_offset={int(cache.storage_offset())} "
            f"storage_elems={storage_elems} -- the per-layer pool view claims "
            "more storage than its backing allocation provides (upstream "
            "KV view emission)"
        )
...
def run_unified_prefill(
    *,
    ...
    Args:
      ...
      workspace:    read ONLY by the warmup page-size guard below (its
                    ``page_size`` scalar is the fallback when the bound view's
                    shape[1] is not a valid SM120 kernel page); otherwise
                    unused -- prefill is single-pass, no split/merge workspace.

    Returns (O[T, heads, D_V=512] bf16, lse[T, heads] f32 base-2).
    """
    # NOTE: ``workspace`` is intentionally NOT deleted here -- the warmup
    # page-size guard below falls back to ``workspace.page_size`` when the
    # bound cache view's shape[1] is not a valid SM120 kernel page (e.g. the
    # minimal (1,1,288) warmup dummy). Deleting it turned that fallback into a
    # NameError on exactly the path the guard exists for.
    ...
    # FAIL-CLOSED before any geometry is derived from the view: a per-layer
    # pool view whose claimed extent exceeds its backing storage would make
    # the MG gather walk to in-view but out-of-allocation addresses -- an
    # async CUDA_ERROR_ILLEGAL_ADDRESS at the next launch instead of an
    # attributable error here.
    _validate_kv_cache_view_storage(kv_cache, "sparse MLA prefill kv_cache")
    if extra_kv_cache is not None:
        _validate_kv_cache_view_storage(
    ...
    if kv_cache.ndim >= 3 and int(kv_cache.shape[1]) in (32, 64, 256):
        ...
        # stride always share the emitted per-layer view's geometry instead
        # of a scratch-plan / manager-block page scalar. [WARMUP GUARD] the
        # minimal warmup bind's shape[1] is not the kernel page (e.g. the
        # (1,1,288) dummy); fall back to the workspace page scalar then
        # (see kernel.py run_unified_decode), and to the CALLER's scalar
        # when no workspace was supplied (prefill accepts workspace=None).
        page_block_size = int(kv_cache.shape[1])
    elif workspace is not None:
        page_block_size = int(workspace.page_size)
    # else: keep the caller's page_block_size scalar (flat 1-D/2-D cache with
    # no workspace: the caller scalar is the only geometry available).
